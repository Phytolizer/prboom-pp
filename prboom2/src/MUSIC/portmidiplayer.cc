...
const music_player_t pm_player = {pm_name, pm_init, nullptr, NULL, NULL, NULL,
...
void pm_stop(void)
{
    ...
    for (i = 0; i < 16; i++)
    {
        ...
        writeevent(when, MIDI_EVENT_CONTROLLER, i, 121,
...
void pm_render(void *vdest, unsigned int bufflen)
{
...
const void *pm_registersong(const void *data, unsigned int len)
{
    midimem_t mf;
...
void pm_shutdown(void)
{
    if (pm_stream)
    {
        ...
        callback function, as doing so can cause a deadlock. Other system
        ...
        some messages still in the windows midi queue when Pm_Close is called.
        this is normally no problem, but if one so happens to dequeue and call
        winmm_streamout_callback at the exact right moment...

        fix: at this point, we've stopped generating midi messages.  sleep for
        more than DRIVER_LATENCY to ensure all messages are flushed.
...
int pm_init(int samplerate)
{
    ...
    if (Pm_OpenOutput(&pm_stream, outputdevice, nullptr, DRIVER_BUFFER, NULL, NULL,
