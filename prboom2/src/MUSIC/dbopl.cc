...
static inline void Operator__UpdateAttenuation(Operator *self)
{
    ...
    self->totalLevel = tl << (ENV_BITS - 7); // Total level goes 2 bits below
                                             // max
...
static void Operator__WriteE0(Operator *self, const Chip *chip, Bit8u val)
{
    ...
    // in opl3 mode you can always selet 7 waveforms regardless of
    // waveformselect
                                ...
                                Bit32s *output, SynthMode mode)
{
    ...
    for (i = 0; i < samples; i++)
    {
        ...
        // Do unsigned shift so we can shift out all bits but still stay in 10
        // bit range otherwise
...
#define REGCHAN(_FUNC_)                                                        \
    ...
    {                                                                          \
        Channel *regChan =                                                     \
            (Channel *)(((char *)self) + ChanOffsetTable[index]);              \
...
void Chip__WriteReg(Chip *self, Bit32u reg, Bit8u val)
{
    ...
    case 0x00 >> 4:
        ...
        else if (reg == 0x105)
        {
            ...
            // MAME says the real opl3 doesn't reset anything on opl3
            // disable/enable till the next write in another register
...
void Chip__Setup(Chip *self, Bit32u rate)
{
    ...
    //-3 since the real envelope takes 8 steps to reach the single value we
    // supply
    for (i = 0; i < 76; i++)
    {
        Bit8u index, shift;
...
void DBOPL_InitTables(void)
{
    ...
    for (i = 0; i < 256; i++)
    {
        ...
        ExpTable[i] =
            (int)(0.5 + (pow(2.0, (255 - i) * (1.0 / 256)) - 1) * 1024);
    ...
    for (i = 0; i < 64; i++)
    {
        ...
        chNum = (i / 8) * 3 + (i % 8) % 3;
        // Make sure we use 16 and up for the 2nd range to match the chanoffset
        // gap
        if (chNum >= 12)
        {
            chNum += 16 - 12;
