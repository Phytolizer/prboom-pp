                             ...
                             mobj_t *soundtarget)
{
    ...
    for (i = 0; i < sec->linecount; i++)
    {
        ...
        other = sides[check->sidenum[sides[check->sidenum[0]].sector == sec]]
                    .sector;
...
static dboolean P_CheckMissileRange(mobj_t *actor)
{
    ...
    if (actor->type == MT_CYBORG || actor->type == MT_SPIDER ||
        actor->type == MT_SKULL || actor->type == HERETIC_MT_IMP)
...
static dboolean P_Move(mobj_t *actor, dboolean dropoff) /* killough 9/12/98 */
{
    ...
    if (friction < ORIG_FRICTION && // sludge
        !(speed = ((ORIG_FRICTION_FACTOR -
                    (ORIG_FRICTION_FACTOR - movefactor) / 2) *
                   speed) /
...
static void P_DoNewChaseDir(mobj_t *actor, fixed_t deltax, fixed_t deltay)
{
    ...
    if (olddir != DI_NODIR &&
        (actor->movedir = static_cast<short>(olddir), P_TryWalk(actor)))
    {
        return;
    }
    ...
    if (P_Random(pr_newchasedir) & 1)
    {
        ...
        {
            if (tdir != turnaround &&
                (actor->movedir = static_cast<short>(tdir), P_TryWalk(actor)))
            {
                return;
            }
        }
    ...
    if ((actor->movedir = static_cast<short>(turnaround)) != DI_NODIR &&
        !P_TryWalk(actor))
    {
        actor->movedir = DI_NODIR;
    }
...
static dboolean PIT_AvoidDropoff(line_t *line)
{
        ...
        P_BoxOnLineSide(tmbbox, line) == -1)
    {
        ...
        {
            angle =
                R_PointToAngle2(0, 0, line->dx, line->dy); // front side dropoff
        }
        else if (front == floorz && back < floorz - FRACUNIT * 24)
        {
            angle =
                R_PointToAngle2(line->dx, line->dy, 0, 0); // back side dropoff
        }
        else
        {
            return true;
...
static void P_NewChaseDir(mobj_t *actor)
{
    ...
    //
    // 1) Stay a certain distance away from a friend, to avoid being in their
    // way 2) Take advantage over an enemy without missiles, by keeping distance
        ...
        { // Random number of monsters, to prevent patterns from forming
            ...
            for (th = cap->cnext; th != cap; th = th->cnext)
            {
                if (--n < 0)
                {
                    ...
                    // the ones which were searched so far, to the end of the
                    // list.
...
void A_PosAttack(mobj_t *actor)
{
    ...
    angle = actor->angle;
    slope =
        P_AimLineAttack(actor, angle, MISSILERANGE, 0); /* killough 8/2/98 */
...
void A_SPosAttack(mobj_t *actor)
{
    ...
    bangle = actor->angle;
    slope =
        P_AimLineAttack(actor, bangle, MISSILERANGE, 0); /* killough 8/2/98 */
    for (i = 0; i < 3; i++)
    { // killough 5/5/98: remove dependence on order of evaluation:
...
static dboolean PIT_VileCheck(mobj_t *thing)
{
    ...
    // Check to see if the radius and height are zero. If they are      //
    // phares then this is a crushed monster that has been turned into a //   |
...
static void A_PainShootSkull(mobj_t *actor, angle_t angle)
{
    ...
    // The original code checked for 20 skulls on the level,            //
    // phares and wouldn't spit another one if there were. If not in // phares
    // compatibility mode, we remove the limit.                         //
    // phares phares
...
void A_Spawn(mobj_t *mo)
{
    ...
    if (mo->state->misc1)
    {
            ...
            !prboom_comp[PC_DO_NOT_INHERIT_FRIENDLYNESS_FLAG_ON_SPAWN].state)
        {
            ...
            newmobj->flags =
                (newmobj->flags & ~MF_FRIEND) | (mo->flags & MF_FRIEND);
        }
    }
}
...
void A_MinotaurCharge(mobj_t *actor)
{
    ...
    {
        puff =
            P_SpawnMobj(actor->x, actor->y, actor->z, HERETIC_MT_PHOENIXPUFF);
...
void Heretic_A_BossDeath(mobj_t *actor)
{
    ...
    static mobjtype_t bossType[6] = {
        HERETIC_MT_HEAD, HERETIC_MT_MINOTAUR, HERETIC_MT_SORCERER2,
        HERETIC_MT_HEAD, HERETIC_MT_MINOTAUR, static_cast<mobjtype_t>(-1)};
