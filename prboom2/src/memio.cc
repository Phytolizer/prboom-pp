...
MEMFILE *mem_fopen_read(const void *buf, size_t buflen)
{
    ...
    file = static_cast<MEMFILE *>(std::malloc(sizeof(MEMFILE)));
...
MEMFILE *mem_fopen_write()
{
    ...
    file->alloced = 1024;
    file->buf = static_cast<unsigned char *>(std::malloc(file->alloced));
    file->buflen = 0;
    file->position = 0;
...
size_t mem_fwrite(const void *ptr, size_t size, size_t nmemb, MEMFILE *stream)
{
    ...
    while (bytes > stream->alloced - stream->position)
    {
        ...
        newbuf = static_cast<unsigned char *>(std::malloc(stream->alloced * 2));
