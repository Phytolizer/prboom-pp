 ...
 * DDCOLORKEY
 */
using DDRAW_H_DDCOLORKEY = struct _DDRAW_H_DDCOLORKEY
{
    unsigned long dwColorSpaceLowValue; // low boundary of color space that is
...
typedef struct _DDRAW_H_DDPIXELFORMAT
{
    ...
    union {
        ...
        unsigned long
            dwPrivateFormatBitCount; // Bits per pixel of private driver
        // formats. Only valid in texture format
    ...
    union {
        ...
        struct
        {
            unsigned short wFlipMSTypes; // Multisample methods supported via
            // flip for this D3DFORMAT
            unsigned short wBltMSTypes; // Multisample methods supported via blt
            // for this D3DFORMAT
        } MultiSampleCaps;
                    ...
                    *bufsize = ddsd.u2.dwMipMapCount > 1
                    ...
                    if (fread(genericImage->pixels, 1, *bufsize, fp) > 0)
                    {
                        ...
                        genericImage->components =
                            (ddsd.u4.ddpfPixelFormat.dwFourCC ==
                             DDRAW_H_FOURCC_DXT1)
                                ? 3
                                : 4;
...
)
{
    ...
    using hires_path_t = struct hires_path_s
    {
...
)
{
    ...
    if (GLEXT_glCompressedTexImage2DARB)
    {
        ...
        GLGenericImage *ddsimage =
            ReadDDSFile(dds_path, &ddsbufsize, &numMipmaps);
        ...
        if (ddsimage)
        {
            ...
            if (tex_width == ddsimage->width && tex_height == ddsimage->height)
            {
                ...
                offset = 0;
                blockSize =
                    (ddsimage->format == GL_COMPRESSED_RGBA_S3TC_DXT1_EXT) ? 8
                                                                           : 16;
                     ...
                     i++)
                {
                    ...
                    size = ((ddsimage->width + 3) / 4) *
                           ((ddsimage->height + 3) / 4) * blockSize;
...
)
{
    ...
    if (access(reinterpret_cast<const char *>(cache_filename), F_OK))
    {
        ...
        if (buf)
        {
            ...
            stat(img_path, &tex_stat);
            cachefp =
                fopen(reinterpret_cast<const char *>(cache_filename), "wb");
            if (cachefp)
            {
            ...
            if (*texid == 0 && gl_texture_internal_hires)
            {
                ...
                if (lumpname)
                {
                    ...
                    if (lump != -1)
                    {
                        ...
                        {
                            SDL_Surface *surf =
                                SDL_ConvertSurface(surf_tmp, &RGBAFormat, 0);
                            SDL_FreeSurface(surf_tmp);
