...
static void R_InitSpriteDefs(const char *const *namelist)
{
    ...
    sprites =
        static_cast<spritedef_t *>(std::calloc(numsprites, sizeof(*sprites)));
    ...
    for (i = 0; (size_t)i < numentries;
         i++) // Prepend each sprite to hash chain
    {         // prepend so that later ones win
        int j =
            R_SpriteNameHash(lumpinfo[i + firstspritelump].name) % numentries;
    ...
    if (drawsegs_xrange_size)
    {
        ...
        while (++curr <= last)
        {
            ...
            if (ds->silhouette & SIL_BOTTOM &&
                spr->gz < ds->bsilheight) // bottom sil
                for (x = r1; x <= r2; x++)
                    if (clipbot[x] == -2)
                        clipbot[x] = ds->sprbottomclip[x];

            if (ds->silhouette & SIL_TOP &&
                spr->gzt > ds->tsilheight) // top sil
                for (x = r1; x <= r2; x++)
                    if (cliptop[x] == -2)
    ...
    if (spr->heightsec != -1) // only things in specially marked sectors
    {
            ...
            (h >>= FRACBITS) < viewheight)
        {
            ...
            { // clip bottom
                ...
                if (phs != -1 &&
                    viewz <= sectors[phs].floorheight) // killough 11/98
                for (x = spr->x1; x <= spr->x2; x++)
...
void R_DrawMasked(void)
{
    ...
    if (num_vissprite > 0)
    {
        ...
        for (ds = ds_p; ds-- > drawsegs;)
        {
            ...
            {
                drawsegs_xranges[0].items[drawsegs_xranges[0].count].x1 =
                    ds->x1;
                drawsegs_xranges[0].items[drawsegs_xranges[0].count].x2 =
                    ds->x2;
