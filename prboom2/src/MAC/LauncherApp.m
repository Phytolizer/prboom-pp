...
- (NSString *)wadPath
{
    return [@"~/Library/Application Support/PrBoom-Plus"
...
- (void)openWebsite:(id)sender
{
    [[NSWorkspace sharedWorkspace]
        openURL:[NSURL URLWithString:@"http://prboom.sourceforge.net/"]];
}
...
- (void)loadDefaults
{
    ...
    [resolutionComboBox
        setObjectValue:[[resolutionComboBox dataSource]
    ...
    if ([defaults boolForKey:@"Saved 2.4.6"])
    {
        ...
        if (NSNotFound !=
            [graphicsModeComboBox indexOfItemWithObjectValue:mode])
            [graphicsModeComboBox
                setObjectValue:[defaults objectForKey:@"Graphics Mode"]];
    }
    ...
    if ([defaults boolForKey:@"Saved 2.4.5"])
    {
        ...
        [[configFileButtonController field]
            setObjectValue:[defaults objectForKey:@"Config File"]];
    }
    ...
    if ([defaults boolForKey:@"Saved"] == true)
    {
        ...
        [respawnMonstersButton
            setObjectValue:[defaults objectForKey:@"Respawn Monsters"]];
        [fastMonstersButton
            setObjectValue:[defaults objectForKey:@"Fast Monsters"]];
        [noMonstersButton
            setObjectValue:[defaults objectForKey:@"No Monsters"]];
        [fullscreenButton
            setObjectValue:[defaults objectForKey:@"Full Screen Graphics"]];
        [disableGraphicsButton
            setObjectValue:[defaults objectForKey:@"Disable Graphics"]];
        [disableJoystickButton
            setObjectValue:[defaults objectForKey:@"Disable Joystick"]];
        [disableMouseButton
            setObjectValue:[defaults objectForKey:@"Disable Mouse"]];
        [disableMusicButton
            setObjectValue:[defaults objectForKey:@"Disable Music"]];
        [disableSoundButton
            setObjectValue:[defaults objectForKey:@"Disable Sound"]];
        [disableSoundEffectsButton
            setObjectValue:[defaults objectForKey:@"Disable Sound Effects"]];
        ...
        long compatIndex =
            [[defaults objectForKey:@"Compatibility Level"] longValue] + 1;
        [compatibilityLevelButton
            setObjectValue:[NSNumber numberWithLong:compatIndex]];
    }
    else
    {
...
- (void)saveDefaults
{
    ...
    [defaults setObject:[graphicsModeComboBox objectValue]
                 forKey:@"Graphics Mode"];

    [window saveFrameUsingName:@"Launcher"];
    [[consoleController window] saveFrameUsingName:@"Console"];

    [defaults setObject:[NSNumber numberWithBool:[wadDrawer state]]
                 forKey:@"Wad Drawer State"];
    [defaults setObject:[NSNumber numberWithBool:[debugDrawer state]]
                 forKey:@"Debug Drawer State"];
    [defaults setObject:[NSNumber numberWithBool:[demoDrawer state]]
                 forKey:@"Demo Drawer State"];

    [defaults setObject:[NSNumber numberWithBool:[[consoleController window]
                                                     isVisible]]
                 forKey:@"Console State"];

    [defaults setObject:[[configFileButtonController field] objectValue]
                 forKey:@"Config File"];

    [defaults setObject:[gameButton objectValue] forKey:@"Game"];
    [defaults setObject:[respawnMonstersButton objectValue]
                 forKey:@"Respawn Monsters"];
    [defaults setObject:[fastMonstersButton objectValue]
                 forKey:@"Fast Monsters"];
    [defaults setObject:[noMonstersButton objectValue] forKey:@"No Monsters"];
    [defaults setObject:[fullscreenButton objectValue]
                 forKey:@"Full Screen Graphics"];
    [defaults setObject:[disableGraphicsButton objectValue]
                 forKey:@"Disable Graphics"];
    [defaults setObject:[disableJoystickButton objectValue]
                 forKey:@"Disable Joystick"];
    [defaults setObject:[disableMouseButton objectValue]
                 forKey:@"Disable Mouse"];
    [defaults setObject:[disableMusicButton objectValue]
                 forKey:@"Disable Music"];
    [defaults setObject:[disableSoundButton objectValue]
                 forKey:@"Disable Sound"];
    [defaults setObject:[disableSoundEffectsButton objectValue]
                 forKey:@"Disable Sound Effects"];
    ...
    [defaults setObject:[NSNumber numberWithLong:compatLevel]
                 forKey:@"Compatibility Level"];
...
- (IBAction)startClicked:(id)sender
{
    ...
    NSString *path =
        [[NSBundle mainBundle] pathForAuxiliaryExecutable:@"PrBoom-Plus"];
    ...
    if ([[[configFileButtonController field] stringValue] length] > 0)
    {
        ...
        [args insertObject:[[configFileButtonController field] stringValue]
                   atIndex:[args count]];
    }
    ...
    for (i = 0; i < [wads count]; ++i)
    {
        ...
        if ([[path pathExtension] caseInsensitiveCompare:@"wad"] ==
            NSOrderedSame)
    ...
    {
        NSString *path = [wads objectAtIndex:i];
        if ([[path pathExtension] caseInsensitiveCompare:@"deh"] ==
            NSOrderedSame)
            [args insertObject:[wads objectAtIndex:i] atIndex:[args count]];
    }
    ...
    if ([demoMatrix selectedCell] != noDemoButton)
    {
        ...
        [args insertObject:[[demoFileButtonController field] stringValue]
                   atIndex:[args count]];

        if ([[ffToLevelField stringValue] length] > 0)
        {
            [args insertObject:@"-ffmap" atIndex:[args count]];
            [args insertObject:[ffToLevelField stringValue]
                       atIndex:[args count]];
        }
    }
...
{
    [[NSWorkspace sharedWorkspace] openFile:[self wadPath]
