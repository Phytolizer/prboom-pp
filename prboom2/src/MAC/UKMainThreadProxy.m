...
- (id)performSelector:(SEL)itemAction
{
    ...
    [target retain];
    [target performSelectorOnMainThread:itemAction
                             withObject:nil
                          waitUntilDone:YES];
    [target release];
...
- (id)mainThreadProxy // Just in case someone accidentally sends this message to
                      // a main thread proxy.
{
    return self;
}

- (id)copyMainThreadProxy // Just in case someone accidentally sends this
                          // message to a main thread proxy.
{
    return [self retain];
}
