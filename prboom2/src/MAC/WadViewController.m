...
- (IBAction)add:(id)sender
{
    ...
    NSArray *types =
        [NSArray arrayWithObjects:@"wad", @"WAD", @"DEH", @"deh", nil];
     ...
     contextInfo:(void *)info
{
    ...
    for (i = 0; i < [[panel filenames] count]; ++i)
        [wads insertObject:[[panel filenames] objectAtIndex:i]
                   atIndex:[wads count]];
...
- (IBAction)remove:(id)sender
{
    ...
    [view selectRowIndexes:[NSIndexSet indexSetWithIndex:-1]
                          ...
                          row:(int)row
{
    ...
    if ([columnId isEqualToString:@"Path"])
        ...
        return [[NSWorkspace sharedWorkspace]
            iconForFile:[wads objectAtIndex:row]];
    else
        return nil;
}
