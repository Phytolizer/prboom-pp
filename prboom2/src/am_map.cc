...
#define F_PANINC                                                               \
    (dsda_InputActive(dsda_input_speed) ? map_scroll_speed * 2                 \
                                        : map_scroll_speed)
...
dboolean AM_Responder(event_t *ev)
{
    ...
    {
        automapmode =
            static_cast<automapmode_e>(automapmode ^ am_grid); // CPhipps
...
static void AM_changeWindowScale()
{
    ...
    {
        float f_paninc =
            (float)F_PANINC / (float)FRACUNIT * (float)tic_vars.frac;
        ...
        {
            mtof_zoommul =
                ((int)((float)FRACUNIT * (1.00f + f_paninc / 200.0f)));
            ftom_zoommul =
                ((int)((float)FRACUNIT / (1.00f + f_paninc / 200.0f)));
        }
...
#define DOOUTCODE(oc, mx, my)                                                  \
    ...
    while (outcode1 | outcode2)
    {
        ...
        else if (outside & BOTTOM)
        {
            ...
            dx = fl->b.x - fl->a.x;
            tmp.x = fl->a.x +
                    (fixed_t)(((int_64_t)dx * (fl->a.y - (f_y + f_h))) / dy);
            tmp.y = f_y + f_h - 1;
            if (am_frame.precise)
            {
                dy_f = fl->a.fy - fl->b.fy;
        ...
        else if (outside & RIGHT)
        {
            ...
            dx = fl->b.x - fl->a.x;
            tmp.y = fl->a.y +
                    (fixed_t)(((int_64_t)dy * (f_x + f_w - 1 - fl->a.x)) / dx);
            tmp.x = f_x + f_w - 1;
            if (am_frame.precise)
            {
...
static void AM_drawMline(mline_t *ml, int color)
{
    ...
    if (color == -1)
    {           // jff 4/3/98 allow not drawing any sort of line
        return; // by setting its color to -1
    }
    if (color == 247)
    { // jff 4/3/98 if color is 247 (xparent), use black
        color = 0;
    }
...
static void AM_drawGrid(int color)
{
    ...
    // Proper math is to just subtract the remainder; AM_drawMLine will take
    // care of clipping if an extra line is offscreen.
    ...
    for (i = 0; i < numlines; i++)
    {
        ...
        if (ddt_cheating || (lines[i].flags & ML_MAPPED))
        {
            ...
            }
            {
                ...
                int amd;
                if (((*mapcolor_bdor_p) || (*mapcolor_ydor_p) ||
                     (*mapcolor_rdor_p)) &&
            ...
            else /* now for 2S lines */
            {
                ...
                if ((*mapcolor_tele_p) && !(lines[i].flags & ML_SECRET) &&
                    (lines[i].special == 39 ||
                     (!heretic &&
                      (lines[i].special == 97 || lines[i].special == 125 ||
                       lines[i].special == 126))))
                { // teleporters
                ...
                else if (lines[i].flags & ML_SECRET) // secret door
                {
                    AM_drawMline(&l, (*mapcolor_wall_p)); // wall color
                         ...
                         !(lines[i].flags &
                           ML_SECRET) && // non-secret closed door
                             ...
                             (map_secret_after &&
                              ((P_WasSecret(lines[i].frontsector) &&
                               ...
                               (P_WasSecret(lines[i].backsector) &&
                                !P_IsSecret(
                                    lines[i].backsector)))) || // jff 3/9/98 add
                                                               // logic to not
                                                               // show secret
                                                               // til after
                                                               // entered
                ...
                {
                    AM_drawMline(&l, (*mapcolor_flat_p)); // 2S lines that
                                                          // appear only in IDDT
                }
            }
    ...
    for (i = 0; i < numsectors; i++)
    {
        ...
        for (pass = 0; pass < 2; pass += (enemies ? 1 : 2))
        {
            ...
            while (t) // for all things in that sector
            {
                ...
                // e6y: stop if all enemies from current sector already has been
                // drawn
                if (pass == 1 && enemies == 0)
                {
                    break;
                ...
                {
                    scale =
                        (BETWEEN(4 << FRACBITS, 256 << FRACBITS, t->radius) >>
                         FRACTOMAPBITS); // * 16 / 20;
                }
                else
                {
                ...
                if ((*mapcolor_rkey_p) || (*mapcolor_ykey_p) ||
                    (*mapcolor_bkey_p))
                {
                    int color = -1;
                    ...
                    if (heretic)
                    {
                        ...
                        case 73: // jff  red key
                            color = (*mapcolor_rkey_p) != -1
                                        ? (*mapcolor_rkey_p)
                                        : (*mapcolor_sprt_p);
                            break;
                        case 80: // jff yellow key
                            color = (*mapcolor_ykey_p) != -1
                                        ? (*mapcolor_ykey_p)
                                        : (*mapcolor_sprt_p);
                            break;
                        case 79: // jff blue key
                            color = (*mapcolor_bkey_p) != -1
                                        ? (*mapcolor_bkey_p)
                                        : (*mapcolor_sprt_p);
                            break;
                        }
        ...
        if (markpoints[i].x != -1)
        {
                    ...
                    : p.y < f_y + f_h && p.y >= f_y)
            {
                ...
                for (k = 0; k < (int)strlen(markpoints[i].label); k++)
                {
                    ...
                    if (p.x < f_x + f_w &&
                        p.x + markpoints[i].widths[k] * SCREENWIDTH / 320 >=
                            f_x)
                    {
                        if (am_frame.precise)
                        {
...
static void AM_setFrameVariables()
{
    float angle;
