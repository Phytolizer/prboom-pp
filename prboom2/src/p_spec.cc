...
struct animdef_t
{
    ...
    char startname[9]{};
    int speed = 0;
} PACKEDATTR; // jff 3/23/98 pack to read from memory
...
fixed_t P_FindNextHighestFloor(sector_t *sec, int currentheight)
{
        ...
        !prboom_comp[PC_FORCE_BOOM_FINDNEXTHIGHESTFLOOR].state)
    {
        ...
        if (!h)
        {
            ...
            // If there are no higher neighbouring sectors, Heretic just
            // returned heightlist[0] (local variable), i.e. noise off the
            // stack. 0 is right for RETURN01 E1M2, so let's take that.
            //
...
fixed_t P_FindShortestTextureAround(int secnum)
{
    ...
    if (!comp[comp_model])
        minsize =
            32000 << FRACBITS; // jff 3/13/98 prevent overflow in height calcs
...
dboolean P_CanUnlockGenDoor(line_t *line, player_t *player)
{
    ...
    case RCard:
            ...
            (!skulliscard || !player->cards[it_redskull]))
        {
            player->message = skulliscard
                                  ? s_PD_REDK
                                  : s_PD_REDC; // Ty 03/27/98 - externalized
            S_StartSound(player->mo, sfx_oof); // killough 3/20/98
            return false;
        }
        break;
    ...
    case RSkull:
            ...
            (!skulliscard || !player->cards[it_redcard]))
        {
            player->message = skulliscard
                                  ? s_PD_REDK
                                  : s_PD_REDS; // Ty 03/27/98 - externalized
            S_StartSound(player->mo, sfx_oof); // killough 3/20/98
            return false;
        }
        break;
    ...
    if (!demo_compatibility) // generalized types not recognized if old demo
    {
        ...
        else if ((unsigned)line->special >= GenFloorBase)
        {
            if (!thing->player && !bossaction)
                if ((line->special & FloorChange) ||
                    ...
                    return; // FloorModel is "Allow Monsters" if FloorChange is
                            // 0
            if (!comperr(comperr_zerotag) &&
                !line->tag) // e6y //jff 2/27/98 all walk generalized types
                            // require tag
                return;
        ...
        else if ((unsigned)line->special >= GenDoorBase)
        {
            if (!thing->player && !bossaction)
            {
                ...
                if (line->flags &
                    ML_SECRET) // they can't open secret doors either
                    return;
            }
            if (!comperr(comperr_zerotag) &&
        ...
        else if ((unsigned)line->special >= GenLockedBase)
        {
            ...
            { // jff 4/1/98 check for being a walk type before reporting door
              // type
...
void P_ShootSpecialLine(mobj_t *thing, line_t *line)
{
    ...
    if (!demo_compatibility)
    {
            ...
            if (!thing->player)
                if ((line->special & FloorChange) ||
                    ...
                    return; // FloorModel is "Allow Monsters" if FloorChange is
                            // 0
            if (!comperr(comperr_zerotag) &&
                !line->tag) // e6y //jff 2/27/98 all gun generalized types
                            // require tag
                return;
        ...
        else if ((unsigned)line->special >= GenDoorBase)
        {
            ...
            if (!comperr(comperr_zerotag) &&
                !line->tag) // e6y //jff 3/2/98 all gun generalized types
                            // require tag
                return;
            linefunc = EV_DoGenDoor;
        }
        else if ((unsigned)line->special >= GenLockedBase)
        {
            ...
            { // jff 4/1/98 check for being a gun type before reporting door
              // type
                if (!P_CanUnlockGenDoor(line, thing->player))
                    return;
            }
                       ...
                       int affectee)
{
    ...
    if (source) // point source exist?
    {
        p->radius = (p->magnitude)
                    << (FRACBITS + 1); // where force goes to zero
        p->x = p->source->x;
        p->y = p->source->y;
    }
    p->affectee = affectee;
