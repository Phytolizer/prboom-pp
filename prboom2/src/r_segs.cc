...
void R_RenderMaskedSegRange(drawseg_t *ds, int x1, int x2)
{
    ...
    for (dcvars.x = x1; dcvars.x <= x2; dcvars.x++, spryscale += rw_scalestep)
    {
        if (maskedtexturecol[dcvars.x] != INT_MAX) // dropoff overflow
        {
            ...
            angle = (ds->rw_centerangle + xtoviewangle[dcvars.x]) >>
                    ANGLETOFINESHIFT;
            dcvars.texu =
            ...
            //
            // sprtopscreen = centeryfrac - FixedMul(dcvars.texturemid,
            // spryscale);
            //
...
static void R_RenderSegLoop()
{
    ...
    for (; rw_x < rw_stopx; rw_x++)
    {
        ...
        if (segtextured)
        {
            ...
            angle_t angle =
                (rw_centerangle + xtoviewangle[rw_x]) >> ANGLETOFINESHIFT;
        ...
        if (midtexture)
        {
            ...
            dcvars.prevsource =
                R_GetTextureColumn(tex_patch, texturecolumn - 1);
            dcvars.nextsource =
                R_GetTextureColumn(tex_patch, texturecolumn + 1);
        ...
        else
        {
            ...
            if (toptexture)
            {
                ...
                if (mid >= yl)
                {
                    ...
                    dcvars.source =
                        R_GetTextureColumn(tex_patch, texturecolumn);
                    dcvars.prevsource =
                        R_GetTextureColumn(tex_patch, texturecolumn - 1);
                    dcvars.nextsource =
                        R_GetTextureColumn(tex_patch, texturecolumn + 1);
...
void R_StoreWallRange(const int start, const int stop)
{
    ...
    rw_normalangle =
        curline->pangle + ANG90; // [crispy] use re-calculated angle
...
#undef ADJUST
    ...
    if (!backsector)
    {
        ...
        rw_midtexturemid +=
            FixedMod(sidedef->rowoffset, textureheight[midtexture]);
    ...
    else // two sided line
    {
        ...
        { /* cph - closed 2S line e.g. door */
            ...
            // required if the fix for the automap bug (r_bsp.c) is used, or
            // else some sprites will be displayed behind closed doors. That fix
            // prevents lines behind closed doors with dropoffs from being
            // displayed on the automap.
    ...
    bottomfrac =
        ((int_64_t)centeryfrac >> invhgtbits) -
    ...
    /* cph - if a column was made solid by this wall, we _must_ save full
     * clipping info */
