    ...
    while (leftout != leftend)
    {
        ...
        for (chan = 0; chan < numChannels; chan++)
        {
            ...
            if (channelinfo[chan].data)
            {
                ...
                // then expanded to 16 bit. this does interpolation and 8->16 at
                // same time, allowing slightly higher quality
...
int I_RegisterSong(const void *data, size_t len)
{
    ...
    if (len > 4 && !music[0])
    {
        ...
        if (result != 0)
        {
            ...
            // haleyjd 04/04/10: scan forward for a MUS header. Evidently DMX
            // was capable of doing this, and would skip over any intervening
            // data. That, or DMX doesn't use the MUS header at all somehow.
            while (musptr <
                   (const unsigned char *)data + len - sizeof(musheader))
            {
                // if we found a likely header start, reset the mus pointer to
                // that location, otherwise just leave it alone and pray.
