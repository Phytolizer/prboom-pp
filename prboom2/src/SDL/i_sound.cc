...
int I_StartSound(int id, int channel, int vol, int sep, int pitch, int priority)
{
    ...
    if ((channel < 0) || (channel >= MAX_CHANNELS))
    {
        ...
        // Example wad: dakills
        // (http://www.doomworld.com/idgames/index.php?id=2803) The entries
        // DSBSPWLK, DSBSPACT, DSSWTCHN and DSSWTCHX are all zero-length sounds
        if (len <= 8)
        {
            return -1;
        }
        ...
        while (leftout != leftend)
        {
            ...
            for (chan = 0; chan < numChannels; chan++)
            {
                ...
                if (channelinfo[chan].data)
                {
                    ...
                    // linear filtering
                    // the old SRC did linear interpolation back into 8 bit, and
                    // then expanded to 16 bit. this does interpolation and
                    // 8->16 at same time, allowing slightly higher quality
    ...
    void I_InitSound(void)
    {
        ...
        if (!use_experimental_music)
        {
            ...
            {
                lprintf(LO_INFO,
                        "couldn't open audio with desired format (%s)\n",
                        SDL_GetError());
                nosfxparm = true;
                nomusicparm = true;
                return;
    ...
    { // assumes 16 bit signed interleaved stereo
        ...
        if (nreq > sinsamp)
        {
            ...
            if (!sinsamp)
            { // avoid pop when first starting stream
                sin[0] = sin[1] = 0;
            }
            sinsamp = nreq;
        }
    ...
    static int Exp_RegisterSongEx(const void *data, size_t len,
                                  int try_mus2mid);
...
#define MUSIC_TMP_EXT (sizeof(music_tmp_ext) / sizeof(*music_tmp_ext))
    ...
    void I_ShutdownMusic(void)
    {
        ...
        if (music_tmp)
        {
            ...
            {
                name = (char *)malloc(strlen(music_tmp) +
                                      strlen(music_tmp_ext[i]) + 1);
    ...
    void I_PauseSong(int handle)
    {
        ...
        case 1:
            ...
            case MUS_NONE:
                break;
            case MUS_MID:
                // SDL_mixer's native MIDI music playing does not pause
                // properly. As a workaround, set the volume to 0 when paused.
                I_SetMusicVolume(0);
                break;
            default:
                Mix_PauseMusic();
    ...
    int I_RegisterSong(const void *data, size_t len)
    {
        ...
        if (len > 4 && !music[0])
        {
            ...
            if (result != 0)
            {
                ...
                // haleyjd 04/04/10: scan forward for a MUS header. Evidently
                // DMX was capable of doing this, and would skip over any
                // intervening data. That, or DMX doesn't use the MUS header at
                // all somehow.
                while (musptr <
                       (const unsigned char *)data + len - sizeof(musheader))
                {
                    // if we found a likely header start, reset the mus pointer
                    // to that location, otherwise just leave it alone and pray.
...
#include "MUSIC/portmidiplayer.hh"
    ...
    static const music_player_t *music_players[] =
        { // until some ui work is done, the order these appear is the
          // autodetect order. of particular importance:  things that play mus
          // have to be last, because mus2midi very often succeeds even on
          // garbage input
...
#define PLAYER_PORTMIDI "portmidi midi player"
    ...
    static int Exp_RegisterSongEx(const void *data, size_t len, int try_mus2mid)
    {
        ...
        if (len > 4 && memcmp(data, "MUS", 3) != 0)
        {
            ...
            for (j = 0; j < NUM_MUS_PLAYERS; j++)
            {
                ...
                for (i = 0; music_players[i]; i++)
                {
                    if (strcmp(music_players[i]->name(),
                               music_player_order[j]) == 0)
                    {
                        found = 1;
                        if (music_player_was_init[i])
                        {
                            const void *temp_handle =
                                music_players[i]->registersong(data, len);
                            if (temp_handle)
                            {
                                SDL_LockMutex(musmutex);
                                current_player = i;
    ...
    void M_ChangeMIDIPlayer(void)
    {
        ...
        else
        {
            ...
            if (!strcasecmp(snd_midiplayer,
                            midiplayers[midi_player_fluidsynth]))
            {
            ...
            else if (!strcasecmp(snd_midiplayer,
                                 midiplayers[midi_player_portmidi]))
            {
