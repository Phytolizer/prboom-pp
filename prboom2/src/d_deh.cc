...
{
    return !fp->lump ? fgetc(fp->f) : fp->size > 0 ? fp->size--,
           *fp->inp++ : EOF;
}
...
void D_BuildBEXTables()
{
    ...
    for (i = 1; i < num_sfx; i++)
    {
        if (!S_sfx[i].names.empty())
        {
            ...
            std::transform(
                S_sfx[i].names.begin(), S_sfx[i].names.end(),
                deh_soundnames[i].begin(),
                [](const auto &sv) {
...
void ProcessDehFile(const char *filename, const char *outfilename, int lumpnum)
{
    ...
    if (outfilename && *outfilename && !fileout)
    {
        ...
        {
            lprintf(
                LO_WARN, "Could not open -dehout file %s\n... using stdout.\n",
                outfilename
            );
            fileout = stdout;
        }
    ...
    while (dehfgets(inbuffer, sizeof(inbuffer), filein))
    {
        ...
        if (!strnicmp(inbuffer, "INCLUDE", 7)) // include a file
        {
            ...
            if (infile.lump)
            {
                ...
                {
                    fprintf(
                        fileout, "No files may be included from wads: %s\n",
                        inbuffer
                    );
                }
                continue;
        ...
        {
            if (!strncasecmp(
                    inbuffer, deh_blocks[i].key, strlen(deh_blocks[i].key)
                ))
            { // matches one
                if (i < DEH_BLOCKMAX - 1)
                {
        ...
        else if (last_i >= 10 && last_i < DEH_BLOCKMAX - 1) // restrict to BEX
                                                            // style lumps
        ...
        {
            fprintf(
                fileout, "Processing function [%d] for %s\n", i,
                deh_blocks[i].key
            );
        }
    ...
    while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' '))
    {
            ...
            (stricmp(key, "FRAME"))) // NOTE: different format from normal
        {
            if (fpout)
            {
                fprintf(
                    fpout,
                    "Invalid BEX codepointer line - must start with "
                    "'FRAME': '%s'\n",
                    inbuffer
                );
            }
        ...
        {
            fprintf(
                fpout, "Processing pointer at index %d: %s\n", indexnum,
                mnemonic
            );
        }
        if (indexnum < 0 || indexnum >= num_states)
        {
            ...
            {
                fprintf(
                    fpout, "Bad pointer number %d of %d\n", indexnum, num_states
                );
            }
        ...
        do      // Ty 05/16/98 - fix loop logic to look for null ending entry
        {
            ...
            { // Ty 06/01/98  - add  to states[].action for new djgcc version
                ...
                {
                    fprintf(
                        fpout, " - applied %s from codeptr[%d] to states[%d]\n",
        ...
        if (!found)
        {
            ...
            {
                fprintf(
                    fpout, "Invalid frame pointer mnemonic '%s' at %d\n",
                    mnemonic, indexnum
                );
            }
        }
...
static uint_64_t getConvertedDEHBits(uint_64_t bits)
{
    static const uint_64_t bitMap[32] = {
        ...
        MF_DROPOFF, // 10 Travels over cliffs - Monsters normally do not walk
        ...
        MF_DROPPED, // 17 Disappearing weapon - Dropped, not spawned (like an
                    // ammo clip) I have not had much success in using this one.
        ...
        MF_TRANSLUCENT // e6y: Translucency via dehacked/bex doesn't work
                       // without it
    };
    int i;
...
static void setMobjInfoValue(int mobjInfoIndex, int keyIndex, uint_64_t value)
{
    ...
    // There is no more synch on
    // http://www.doomworld.com/sda/dwdemo/w303-115.zip (with correction in
    // PIT_CheckThing)
    case 22:
    ...
    case 23:
        ...
        break;
    case 24:
        mi->droppeditem = static_cast<mobjtype_t>((int)(value - 1));
...
static void deh_procThing(DEHFILE *fpin, FILE *fpout, char *line)
{
    ...
    while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' '))
    {
        ...
        if (!bGetData)
        // Old code: if (!deh_GetData(inbuffer,key,&value,&strval,fpout)) //
        // returns TRUE if ok
        {
            if (fpout)
            {
        ...
        for (ix = 0; ix < DEH_MOBJINFOMAX; ix++)
        {
            ...
            if (deh_strcasecmp(key, "Bits"))
            {
                ...
                // The old code here was the cause of a DEH-related bug in
                // prboom. When the mobjinfo_t.flags member was graduated to an
                // int64, this code was caught unawares and was indexing each
                // property of the mobjinfo as if it were still an int32. This
                // caused sets of the "raisestate" member to partially overwrite
                // the "flags" member, thus screwing everything up and making
                // most DEH patches result in unshootable enemy types. Moved to
                // a separate function above and stripped of all hairy struct
                // address indexing. - POPE
                ...
                else
                {
                         ...
                         strval = nullptr)
                    {
                        ...
                        for (iy = 0; iy < DEH_MOBJFLAGMAX; iy++)
                        {
                            ...
                            {
                                fprintf(
                                    fpout, "ORed value 0x%08lX%08lX %s\n",
                                    (unsigned long
                                    )(deh_mobjflags[iy].value >> 32) &
                                        0xffffffff,
                                    (unsigned long)deh_mobjflags[iy].value &
                                        0xffffffff,
                                    strval
                                );
                            }
                        ...
                        {
                            fprintf(
                                fpout, "Could not find bit mnemonic %s\n",
                                strval
                            );
                        }
                    }
            ...
            {
                fprintf(
                    fpout, "Assigned 0x%08lx%08lx to %s(%d) at index %d\n",
                    (unsigned long)(value >> 32) & 0xffffffff,
...
static void deh_procFrame(DEHFILE *fpin, FILE *fpout, char *line)
{
    ...
    while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' '))
    {
                 ...
                 )) // Codep frame (not set in Frame deh block)
        {
            ...
            {
                fprintf(
                    fpout, " - codep, should not be set in Frame section!\n"
                );
            }
            /* nop */;
        }
...
static void deh_procPointer(DEHFILE *fpin, FILE *fpout, char *line) // done
{
    ...
    while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' '))
    {
        ...
        if (value >= static_cast<uint_64_t>(num_states))
        {
            ...
            {
                fprintf(
                    fpout, "Bad pointer number %ld of %d\n", (long)value,
                    num_states
                );
            }
            return;
            ...
            )) // Codep frame (not set in Frame deh block)
        {
            ...
            {
                if (!memcmp(
                        &deh_bexptrs[i].cptr, &deh_codeptr[value],
                    ...
                    ))
                {
                    ...
                    {
                        fprintf(
                            fpout, "BEX [CODEPTR] -> FRAME %d = %s\n", indexnum,
                            &deh_bexptrs[i].lookup[2]
                        );
                    }
                    break;
                }
                if (deh_bexptrs[i].cptr.isNull())
                { // stop at null entry
                    break;
                }
            }
        ...
        {
            fprintf(
                fpout, "Invalid frame pointer index for '%s' at %ld\n", key,
                (long)value
            );
        }
    }
...
static void deh_procSounds(DEHFILE *fpin, FILE *fpout, char *line)
{
    ...
    while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' '))
    {
        ...
        if (!deh_strcasecmp(key, deh_sfxinfo[0]))
        {              // Offset
            /* nop */; // we don't know what this is, I don't think
        }
        else if (!deh_strcasecmp(key, deh_sfxinfo[1]))
        { // Zero/One
            S_sfx[indexnum].singularity = (int)value;
        }
        else if (!deh_strcasecmp(key, deh_sfxinfo[2]))
        { // Value
            S_sfx[indexnum].priority = (int)value;
        }
        else if (!deh_strcasecmp(key, deh_sfxinfo[3]))
        { // Zero 1
            // S_sfx[indexnum].link = (sfxinfo_t *)value;
            ; // .link - don't set pointers from DeHackEd
        }
        else if (!deh_strcasecmp(key, deh_sfxinfo[4]))
        { // Zero 2
            S_sfx[indexnum].pitch = (int)value;
        }
        else if (!deh_strcasecmp(key, deh_sfxinfo[5]))
        { // Zero 3
            S_sfx[indexnum].volume = (int)value;
        }
        else if (!deh_strcasecmp(key, deh_sfxinfo[6]))
        { // Zero 4
            // S_sfx[indexnum].data = (void *) value; // killough 5/3/98:
            // changed cast
            ; // .data - don't set pointers from DeHackEd
        }
        else if (!deh_strcasecmp(key, deh_sfxinfo[7]))
        { // Neg. One 1
...
static void deh_procAmmo(DEHFILE *fpin, FILE *fpout, char *line)
{
    ...
    while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' '))
    {
        ...
        if (!deh_strcasecmp(key, deh_ammo[0]))
        { // Max ammo
            maxammo[indexnum] = (int)value;
        }
        else if (!deh_strcasecmp(key, deh_ammo[1]))
        { // Per ammo
...
static void deh_procWeapon(DEHFILE *fpin, FILE *fpout, char *line)
{
    ...
    while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' '))
    {
        ...
        else if (!deh_strcasecmp(key, deh_weapon[1]))
        { // Deselect frame
            weaponinfo[indexnum].upstate = (int)value;
        }
        else if (!deh_strcasecmp(key, deh_weapon[2]))
        { // Select frame
            weaponinfo[indexnum].downstate = (int)value;
        }
        else if (!deh_strcasecmp(key, deh_weapon[3]))
        { // Bobbing frame
            weaponinfo[indexnum].readystate = (int)value;
        }
        else if (!deh_strcasecmp(key, deh_weapon[4]))
        { // Shooting frame
            weaponinfo[indexnum].atkstate = (int)value;
        }
        else if (!deh_strcasecmp(key, deh_weapon[5]))
        { // Firing frame
...
// Returns: void
//
static void deh_procSprite(
    DEHFILE *fpin, FILE *fpout,
    char *line
) // Not supported
{
    char key[DEH_MAXKEYLEN];
    ...
    {
        fprintf(
            fpout, "Ignoring Sprite offset change at index %d: %s\n", indexnum,
            key
        );
    }
    while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' '))
    ...
    while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' '))
    {
        ...
        { // not 3
            ...
            { // not 2
                ...
                {
                    fprintf(
                        fpout, "Invalid par time setting string: %s\n", inbuffer
                    );
                }
            }
            else
            { // is 2
                ...
                else
                {
                    ...
                    if (fpout)
                    {
                        fprintf(
                            fpout,
                            "Changed par time for MAP%02d from %d to %d\n",
        ...
        { // is 3
            ...
            if (episode < 1 || episode > 3 || level < 1 || level > 9)
            {
                ...
                {
                    fprintf(
                        fpout, "Invalid ExMx values E%dM%d\n", episode, level
                    );
                }
            }
            else
            {
                ...
                {
                    fprintf(
                        fpout, "Changed par time for E%dM%d from %d to %d\n",
        ...
        for (ix = 0; cheat[ix].cheat; ix++)
        {
            if (cheat[ix].deh_cheat) // killough 4/18/98: skip non-deh
            {
                    ...
                    )) // found the cheat, ignored case
                {
                    ...
                    {
                        strval[iy] =
                            (strval[iy] == (char)0xff) ? '\0' : strval[iy];
                    }
          ...
          cheat[i].deh_modified = true;
                    ...
                    if (deh_apply_cheats && !M_CheckParm("-nocheats"))
                    {
                        ...
                        if (fpout)
                        {
                            fprintf(
                                fpout,
                                "Assigned new cheat '%s' to cheat '%s'at "
                                "index %d\n",
...
static void deh_procMisc(DEHFILE *fpin, FILE *fpout, char *line) // done
{
    ...
    while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' '))
    {
        ...
        if (!deh_strcasecmp(key, deh_misc[0]))
        { // Initial Health
            initial_health = (int)value;
        }
        else if (!deh_strcasecmp(key, deh_misc[1]))
        { // Initial Bullets
            initial_bullets = (int)value;
        }
        else if (!deh_strcasecmp(key, deh_misc[2]))
        {                                                      // Max Health
            IsDehMaxHealth = true, deh_maxhealth = (int)value; // e6y
        }
        else if (!deh_strcasecmp(key, deh_misc[3]))
        { // Max Armor
            max_armor = (int)value;
        }
        else if (!deh_strcasecmp(key, deh_misc[4]))
        { // Green Armor Class
            green_armor_class = (int)value;
        }
        else if (!deh_strcasecmp(key, deh_misc[5]))
        { // Blue Armor Class
            blue_armor_class = (int)value;
        }
        else if (!deh_strcasecmp(key, deh_misc[6]))
        {                                                   // Max Soulsphere
            IsDehMaxSoul = true, deh_max_soul = (int)value; // e6y
        }
        else if (!deh_strcasecmp(key, deh_misc[7]))
        { // Soulsphere Health
            soul_health = (int)value;
        }
        else if (!deh_strcasecmp(key, deh_misc[8]))
        { // Megasphere Health
            IsDehMegaHealth = true, deh_mega_health = (int)value; // e6y
        }
        else if (!deh_strcasecmp(key, deh_misc[9]))
        { // God Mode Health
            god_health = (int)value;
        }
        else if (!deh_strcasecmp(key, deh_misc[10]))
        { // IDFA Armor
            idfa_armor = (int)value;
        }
        else if (!deh_strcasecmp(key, deh_misc[11]))
        { // IDFA Armor Class
            idfa_armor_class = (int)value;
        }
        else if (!deh_strcasecmp(key, deh_misc[12]))
        { // IDKFA Armor
            idkfa_armor = (int)value;
        }
        else if (!deh_strcasecmp(key, deh_misc[13]))
        { // IDKFA Armor Class
            idkfa_armor_class = (int)value;
        }
        else if (!deh_strcasecmp(key, deh_misc[14]))
        { // BFG Cells/Shot
            bfgcells = (int)value;
        }
        ...
        { // Monsters Infight
            ...
            {
                fprintf(
                    fpout, "Invalid value for 'Monsters Infight': %i",
                    (int)value
                );
            }
...
{
    char key[DEH_MAXKEYLEN];
    char inbuffer[DEH_BUFFERMAX * 2]; // can't use line -- double size buffer
                                      // too.
    int i;                            // loop variable
    ...
    {
        fprintf(
            fpout, "Processing Text (key=%s, from=%d, to=%d)\n", key, fromlen,
            tolen
        );
    }
        ...
        tolen < 7) // lengths of music and sfx are 6 or shorter
    {
        ...
        for (i = 1; i < num_sfx; i++)
        {
                ...
                !S_sfx_state[i])
            {
                ...
                {
                    fprintf(
                        fpout, "Changing name of sfx from %s to %*s\n",
                        S_sfx[i].names[0].data(), usedlen, &inbuffer[fromlen]
                    );
                }
        ...
        if (!found) // not yet
        {
            ...
            for (i = 1; i < num_music; i++)
            {
                    ...
                    !S_music_state[i])
                {
                    ...
                    {
                        fprintf(
                            fpout, "Changing name of music from %s to %*s\n",
                ...
                // it.
    {
        if (fpout)
        {
            fprintf(
                fpout,
                "Checking text area through strings for '%.12s%s' from=%d "
                "to=%d\n",
    ...
    while (!dehfeof(fpin) && *inbuffer) /* && (*inbuffer != ' ') */
    {
        ...
        ) // Ty03/29/98 - fix stupid error
        {
            ...
            holdstring = static_cast<char *>(
                std::realloc(holdstring, maxstrlen * sizeof(*holdstring))
            );
        }
        ...
        if (*holdstring) // didn't have a backslash, trap above would catch that
        {
            ...
            found = deh_procStringSub(
                key, nullptr, holdstring,
                fpout
            ); // supply keyand not search string
...
//
dboolean deh_procStringSub(
    char *key, char *lookfor, char *newstring, FILE *fpout
)
{
    ...
    for (i = 0; i < deh_numstrlookup; i++)
    {
        ...
        if (found)
        {
            ...
            *deh_strlookup[i].ppstr = t =
            ...
            if (!key)
            { // must have passed an old style string so showBEX
                if (fpout)
                {
                    fprintf(
                        fpout, "*BEX FORMAT:\n%s = %s\n*END BEX\n",
    ...
    while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' '))
    {
        ...
        while (deh_spritenames[rover])
        {
            if (!strncasecmp(deh_spritenames[rover], key, 4))
            {
                ...
                {
                    fprintf(
                        fpout, "Substituting '%s' for sprite '%s'\n", candidate,
    ...
    while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' '))
    {
        ...
        while (!deh_soundnames[rover].empty())
        {
            if (!strncasecmp(deh_soundnames[rover][0].c_str(), key, 6))
            {
                ...
                {
                    fprintf(
                        fpout, "Substituting '%s' for sound '%s'\n", candidate,
                        deh_soundnames[rover][0].c_str()
                    );
                }
    ...
    while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' '))
    {
        ...
        while (deh_musicnames[rover])
        {
            if (!strncasecmp(deh_musicnames[rover], key, 6))
            {
                ...
                {
                    fprintf(
                        fpout, "Substituting '%s' for music '%s'\n", candidate,
...
dboolean
deh_GetData(char *s, char *k, uint_64_t *l, char **strval, FILE * /* fpout */)
{
    char *t;                    // current char
