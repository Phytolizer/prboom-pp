        ...
        else
        {
            ...
            if (mlook_lump.lump != -1)
            {
                const unsigned char *data =
                    W_CacheLumpName<const unsigned char *>(mlook_lump.name);
...
static void R_DemoEx_GetParams(const byte *pwad_p, waddata_t *waddata)
{
    ...
    if (params)
    {
        ...
        // overflows
        {
            ...
            for (overflow = static_cast<overrun_list_t>(0);
                 overflow < OVERFLOW_MAX;
                 overflow = static_cast<overrun_list_t>(overflow + 1))
            {
                int value;
                char *pstr, *mask;
...
static void R_DemoEx_AddParams(wadtbl_t *wadtbl)
{
    ...
    {
        overrun_list_t overflow;
        for (overflow = static_cast<overrun_list_t>(0); overflow < OVERFLOW_MAX;
             overflow = static_cast<overrun_list_t>(overflow + 1))
        {
            ...
            if (header->infotableofs + length <= size)
            {
                ...
                for (i = 0; i < header->numlumps; i++, fileinfo++)
                {
                    if (fileinfo->filepos < 0 ||
                        ...
                        fileinfo->filepos + fileinfo->size >
                            header->infotableofs)
                    {
                        break;
                    }
...
#else
    {
        ...
        for (i = 0; (size_t)i < numwadfiles; i++)
        {
            if (wadfiles[i].src == source_lmp)
            {
                ...
                {
                    result = true;
                    if ((size_t)numwadfiles_required + 1 !=
                            waddata.numwadfiles &&
                        patterndata.missed)
                    {
