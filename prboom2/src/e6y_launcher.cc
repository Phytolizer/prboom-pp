...
static void L_FilesOnChange(void)
{
    ...
    if (index != LB_ERR)
    {
        ...
        if (index != LB_ERR)
        {
            ...
            font =
                (HFONT)SendMessage(launcher.staticFileName, WM_GETFONT, 0, 0);
...
static dboolean L_GetFileType(const char *filename, fileitem_t *item)
{
    ...
    if ((f = fopen(filename, "rb")))
    {
        ...
        if (item->source != source_err)
        {
            ...
            {
                for (i = 0; !item->doom1 && !item->doom2 &&
                            i < (size_t)header.numlumps;
                     i++)
                {
                    filelump_t lump;
...
static dboolean L_PrepareToLaunch(void)
{
    ...
    for (i = 0; (size_t)i < new_numwadfiles; i++)
    {
        if (new_wadfiles[i].src == source_lmp ||
    ...
    for (i = launcher.selectioncount - 1; i >= 0; i--)
    {
        ...
        if (!present)
        {
            for (j = 0; j < listPWADCount; j++)
            {
                ...
                if (SendMessage(launcher.listPWAD, LB_GETITEMDATA, j, 0) ==
                    index)
                {
                            ...
                            if (item.source != source_err)
                            {
                                ...
                                if (!present)
                                {
                                    ...
                                    launcher.files[launcher.filescount].doom1 =
                                        item.doom1;
                                    launcher.files[launcher.filescount].doom2 =
                                        item.doom2;
                                    launcher.filescount++;
                                }
                            }
                                     ...
                                     LPARAM lParam)
{
    ...
    case WM_INITDIALOG: {
        ...
        launcher.listHistory =
            GetDlgItem(launcher.HWNDClient, IDC_HISTORYCOMBO);
    ...
    if ((((LPNMHDR)lParam)->code) == TTN_NEEDTEXT)
    {
        ...
        case IDC_PWADLIST: {
            ...
            for (i = 0; i < selectioncount; i++)
            {
                ...
                int needlen = (tooltip_str ? strlen(tooltip_str) : 0) +
                              strlen(line) + sizeof(char) * 8;
