...
static void gld_PrecalculateSector(int num)
{
    ...
    while (linecount)
    {
        ...
        linecount--;
        // find the next line
        oldline = currentline; // if this isn't changed at the end of the
                               // search, a new loop will start
            ...
            { // if the line isn't already added ...
                    ...
                    (sectors[num].lines[i]->v2 == currentvertex))
                {
                    ...
                    if (bestline == -1) // if this is the first one ...
                    {
                        ...
                        // check if the angle between the current line and this
                        // best line candidate is smaller then the angle of the
                        // last candidate e6y: for finding an angle between AB
                        // and BC vectors we should subtract (BC - BA) == (BC -
...
static void gld_GetSubSectorVertices(void)
{
    ...
    for (i = 0; i < numsubsectors; i++)
    {
        ...
        if (flats_vbo)
        {
            ...
            {
                flats_vbo[gld_num_vertexes].u =
                    ((float)(segs[ssector->firstline + j].v1->x) / FRACUNIT) /
                    64.0f;
                flats_vbo[gld_num_vertexes].v =
                    (-(float)(segs[ssector->firstline + j].v1->y) / FRACUNIT) /
                    64.0f;
                flats_vbo[gld_num_vertexes].x =
...
static void gld_MarkSectorsForClamp(void)
{
    ...
    for (i = 0; i < numsectors; i++)
    {
        ...
        for (loopnum = 0; !fail && loopnum < sectorloops[i].loopcount;
             loopnum++)
        {
            int vertexnum;
...
static void gld_PreprocessSegs(void)
{
    ...
    gl_segs = static_cast<GLSeg *>(std::malloc(numsegs * sizeof(GLSeg)));
    for (i = 0; i < numsegs; i++)
    {
    ...
    gl_lines = static_cast<GLSeg *>(std::malloc(numlines * sizeof(GLSeg)));
