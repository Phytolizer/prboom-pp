...
dboolean P_SightTraverseIntercepts()
{
    int count;
    fixed_t dist;
                                   ...
                                   fixed_t y2, divline_t *node)
{
    ...
    if (!node->dy)
    {
        ...
        // if ((compatibility_level < prboom_4_compatibility ? x1 : y1) ==
        // node->y)
        //  return (x2 == node->y);
        if (_y1 == node->y)
            return (_y2 == node->y);
...
dboolean P_CrossSubsector_PrBoom(int num)
{
    ...
    const sector_t *front = nullptr, *back = nullptr;
...
static dboolean P_CrossBSPNode_PrBoom(int bspnum)
{
    while (!(bspnum & NF_SUBSECTOR))
    {
        ...
        int side, side2;
        side =
            P_DivlineSide(los.strace.x, los.strace.y, (const divline_t *)bsp) &
            1;
...
dboolean P_CheckSight(mobj_t *t1, mobj_t *t2)
{
    ...
    los.topslope =
        (los.bottomslope = t2->z - (los.sightzstart = t1->z + t1->height -
                                                      (t1->height >> 2))) +
        t2->height;
