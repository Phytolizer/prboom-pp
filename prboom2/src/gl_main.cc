...
void gld_InitTextureParams()
{
    ...
    int i;
    int *var[MIP_COUNT] = {
        &gl_texture_filter, &gl_sprite_filter, &gl_patch_filter};
    ...
    while (tex_formats[i].tex_format_name)
    {
        if (!strcasecmp(gl_tex_format_string, tex_formats[i].tex_format_name))
        {
            ...
            lprintf(
                LO_INFO, "Using texture format %s.\n",
...
void gld_MultisamplingCheck()
{
    if (render_multisampling)
    {
        ...
        if (test != render_multisampling)
        {
            ...
            rust::save_defaults();
            I_Error(
                "Couldn't set %dX multisamples for %dx%d video mode", i,
...
)
{
    ...
    alpha = (float
            )((automapmode & am_overlay) ? map_textured_overlay_trans
                                         : map_textured_trans) /
            100.0f;
    if (alpha == 0)
    {
    ...
    if (numsubsectors > visible_subsectors_size)
    {
        ...
        visible_subsectors = static_cast<subsector_t **>(realloc(
            visible_subsectors,
    ...
    if (visible_subsectors_count != visible_subsectors_count_prev)
    {
        ...
        qsort(
            visible_subsectors, visible_subsectors_count,
    ...
    coord_scale = (float)scale / (float)(1 << FRACTOMAPBITS) / (float)FRACUNIT *
                  MAP_COEFF;
    ...
    for (i = 0; i < visible_subsectors_count; i++)
    {
        ...
        gltexture =
            gld_RegisterFlat(flattranslation[sub->sector->floorpic], true);
        if (gltexture)
        {
            sector_t tempsec;
            int floorlight;
            ...
            for (loopnum = 0; loopnum < subsectorloops[ssidx].loopcount;
                 loopnum++)
            {
                int vertexnum;
                ...
                {
                    glTexCoord2f(
                        flats_vbo[vertexnum].u + floor_uoffs,
                        flats_vbo[vertexnum].v + floor_voffs
                    );
                    glVertex3f(
                        flats_vbo[vertexnum].x, flats_vbo[vertexnum].z, 0
                    );
                }
                glEnd();
            }
...
void gld_DrawNumPatch_f(
    float x, float y, int lump, int cm, enum patch_translation_e flags
)
{
    GLTexture *gltexture;
    ...
    if (flags & VPT_STRETCH_MASK)
    {
        ...
        ypos = (float)((y - topoffset) * params->video->height) / 200.0f +
               params->deltay1;
        width =
            (float)(gltexture->realtexwidth * params->video->width) / 320.0f;
        height =
            (float)(gltexture->realtexheight * params->video->height) / 200.0f;
    }
    else
    {
...
void gld_DrawNumPatch(
    int x, int y, int lump, int cm, enum patch_translation_e flags
)
{
...
void gld_FillFlat(
    int lump, int x, int y, int width, int height,
...
void gld_DrawLine_f(float x0, float y0, float x1, float y1, int BaseColor)
{
    ...
    a =
        ((automapmode & am_overlay) ? map_lines_overlay_trans * 255 / 100 : 255
        );
    if (a == 0)
    {
        return;
    ...
    map_line_t &line = map_lines.back();
    ...
    glColor4f(
        (float)playpal[3 * BaseColor] / 255.0f,
...
void gld_FillBlock(int x, int y, int width, int height, int col)
{
    ...
    glColor3f(
        (float)playpal[3 * col] / 255.0f, (float)playpal[3 * col + 1] / 255.0f,
...
void gld_SetPalette(int palette)
{
    ...
    if (gl_shared_texture_palette)
    {
        ...
        GLEXT_glColorTableEXT(
            GL_SHARED_TEXTURE_PALETTE_EXT, GL_RGBA, 256, GL_RGBA,
...
{ // NSM convert to static
    ...
    if (buffer && scr)
    {
        ...
        glFlush();
        glReadPixels(
            0, 0, SCREENWIDTH, SCREENHEIGHT, GL_RGB, GL_UNSIGNED_BYTE, scr
        );
        ...
        for (i = 0; i < SCREENHEIGHT / 2; i++)
        {
            ...
            memcpy(
                &scr[i * SCREENWIDTH * 3],
                &scr[(SCREENHEIGHT - (i + 1)) * SCREENWIDTH * 3],
                SCREENWIDTH * 3
            );
            memcpy(
                &scr[(SCREENHEIGHT - (i + 1)) * SCREENWIDTH * 3], buffer,
                SCREENWIDTH * 3
            );
        }
    }
...
GLvoid gld_Set2DMode()
{
    ...
    glLoadIdentity();
    glOrtho(
        (GLdouble)0, (GLdouble)SCREENWIDTH, (GLdouble)SCREENHEIGHT, (GLdouble)0,
...
void gld_StartDrawScene()
{
    ...
    glScissor(
        viewwindowx, SCREENHEIGHT - (viewheight + viewwindowy), viewwidth,
        viewheight
    );
    glEnable(GL_SCISSOR_TEST);
    // Player coordinates
...
void gld_EndDrawScene()
{
    ...
    if (SceneInTexture)
    {
        ...
        if (motion_blur.enabled)
        {
            ...
            if (realframe)
            {
                motionblur_alpha = (float
                                   )((atan(-renderer_fps / motion_blur.att_a)) /
                                     motion_blur.att_b) +
                                   motion_blur.att_c;
            }
...
static void gld_DrawWall(GLWall *wall)
{
    ...
    has_detail = scene_has_details && gl_arb_multitexture &&
                 (wall->flag < GLDWF_SKY) && (wall->gltexture->detail) &&
                 gld_IsDetailVisible(
                     xCamera, yCamera, wall->glseg->x1, wall->glseg->z1,
...
#define CALC_TEX_VALUES_BOTTOM(                                                \
    w, seg, backseg, peg, linelength, lineheight, v_offset                     \
)                                                                              \
    (w).flag = GLDWF_BOT;                                                      \
    URUL(w, seg, backseg, linelength);                                         \
    if (peg)                                                                   \
...
void gld_AddWall(seg_t *seg)
{
    ...
    frontsector = R_FakeFlat(
        seg->frontsector, &ftempsec, nullptr, nullptr,
        false
    ); // for boom effects
    if (!frontsector)
    {
    ...
    wall.light = gld_CalcLightLevel(
        frontsector->lightlevel + rellight + (extralight << 5)
    );
    ...
    if (!seg->backsector) /* onesided */
    {
        if (frontsector->ceilingpic == skyflatnum)
        {
            ...
            gld_AddSkyTexture(
                &wall, frontsector->sky, frontsector->sky, SKY_CEILING
            );
        }
        if (frontsector->floorpic == skyflatnum)
        {
            ...
            gld_AddSkyTexture(
                &wall, frontsector->sky, frontsector->sky, SKY_FLOOR
            );
        }
        temptex = gld_RegisterTexture(
            texturetranslation[seg->sidedef->midtexture], true, false
        );
        if (temptex && frontsector->ceilingheight > frontsector->floorheight)
        {
            wall.gltexture = temptex;
            CALC_Y_VALUES(
                wall, lineheight, frontsector->floorheight,
    ...
    else /* twosided */
    {
        ...
        backsector = R_FakeFlat(
            seg->backsector, &btempsec, nullptr, nullptr,
            true
        ); // for boom effects
        if (!backsector)
        {
            ...
            skyflatnum) // || backsector->ceilingpic==skyflatnum)
        {
            ...
            if (
                // e6y
                // There is no more HOM in the starting area on Memento Mori
                // map29 and on map30. Old code:
            ...
            {
                // e6y
                // There is no more visual glitches with sky on Icarus map14
                // sector 187 Old code:
                // wall.ybottom=(float)backsector->floorheight/MAP_SCALE;
                wall.ybottom =
                    ((float
                    )(backsector->floorheight + (seg->sidedef->rowoffset > 0
                                                     ? seg->sidedef->rowoffset
                                                     : 0))) /
                    MAP_SCALE;
                gld_AddSkyTexture(
                    &wall, frontsector->sky, backsector->sky, SKY_CEILING
                );
            }
            else
            {
                if (bs->ceilingpic == skyflatnum &&
                    fs->ceilingpic != skyflatnum && toptexture == NO_TEXTURE &&
                ...
                else
                {
                        ...
                        backsector->ceilingheight <= frontsector->floorheight)
                    {
                        ...
                        gld_AddSkyTexture(
                            &wall, frontsector->sky, backsector->sky,
                            SKY_CEILING
                        );
                    }
                }
        ...
        if (floor_height < ceiling_height)
        {
                  ...
                  (backsector->ceilingpic == skyflatnum)))
            {
                ...
                else if (temptex)
                {
                    wall.gltexture = temptex;
                    CALC_Y_VALUES(
                        wall, lineheight, floor_height, ceiling_height
                    );
                    CALC_TEX_VALUES_TOP(
                        wall, seg, backseg,
                        (LINE->flags &
                         (/*e6y ML_DONTPEGBOTTOM | */ ML_DONTPEGTOP)) == 0,
                        linelength, lineheight
                    );
                    gld_AddDrawWallItem(GLDIT_WALL, &wall);
                }
        ...
        {
            temptex =
                gld_RegisterTexture(seg->sidedef->midtexture, true, false);
        }
        else
        {
            ...
            // See spacelab.wad
            // (http://www.doomworld.com/idgames/index.php?id=6826)
            ...
            backsector->ceilingheight > frontsector->floorheight)
        {
            ...
            if ((LINE->flags & ML_DONTPEGBOTTOM) > 0)
            {
                ...
                floor_height = MAX(seg->frontsector->floorheight,
            ...
            else
            {
                ...
                floor_height = ceiling_height -
                               (wall.gltexture->realtexheight << FRACBITS);
            }
                ...
                seg->frontsector->heightsec != -1)
            {
                ...
                f =
                    (seg->frontsector->heightsec == -1 ? seg->frontsector
                                                       : &ftempsec);
                b =
                    (seg->backsector->heightsec == -1 ? seg->backsector
                                                      : &btempsec);
            ...
            gld_AddDrawWallItem(
                (wall.alpha == 1.0f ? GLDIT_MWALL : GLDIT_TWALL), &wall
            );
            wall.alpha = 1.0f;
        }
    bottomtexture:
        ...
        if (frontsector->floorpic == skyflatnum)
        {
                ...
                (backsector->floorpic == skyflatnum))
            {
                ...
                gld_AddSkyTexture(
                    &wall, frontsector->sky, backsector->sky, SKY_FLOOR
                );
            }
            else
            {
                if (bs->floorpic ==
                        skyflatnum && // fs->floorpic != skyflatnum &&
        ...
        if (floor_height < ceiling_height)
        {
            ...
            {
                wall.ytop =
                    ((float)(ceiling_height) / (float)MAP_SCALE) + SMALLDELTA;
                wall.ybottom =
                    ((float)(floor_height) / (float)MAP_SCALE) - SMALLDELTA;
                if (wall.ytop <= zCamera)
                {
                    ...
                    temptex = gld_RegisterFlat(
                        flattranslation[seg->backsector->floorpic], true
                    );
                    if (temptex)
                    {
                        wall.gltexture = temptex;
    ...
    if (flat->sectornum >= 0)
    {
        ...
        else
        {
                 ...
                 loopnum++)
            {
                ...
                glDrawArrays(
                    currentloop->mode, currentloop->vertexindex,
             ...
             loopnum++)
        {
                 ...
                 vertexnum++)
            {
                ...
                glVertex3f(
                    flats_vbo[vertexnum].x, flat->z, flats_vbo[vertexnum].z
                );
            }
            // end of loop
            glEnd();
...
static void gld_AddFlat(int sectornum, dboolean ceiling, visplane_t *plane)
{
    ...
    sector = R_FakeFlat(
        sector, &tempsec, &floorlightlevel, &ceilinglightlevel,
        false
    ); // for boom effects
    ...
    gld_AddDrawItem(
        ((flat.flags & GLFLAT_CEILING) ? GLDIT_CEILING : GLDIT_FLOOR), &flat
    );
}
        ...
        else
        {
            if (sprite->flags & g_mf_translucent)
            {
                gld_StaticLightAlpha(
                    sprite->light, (float)tran_filter_pct / 100.0f
                );
            }
            else
            {
...
static void gld_AddHealthBar(mobj_t *thing, GLSprite *sprite)
{
        ...
        (thing->health > 0))
    {
        ...
        if (hbar.cm >= 0)
        {
            ...
            float sx1 = sx2 - (float)health_percent * (float)thing->radius /
                                  100.0f / MAP_SCALE;
            float sx3 = -sx2;
...
void gld_ProjectSprite(mobj_t *thing, int lightlevel)
{
    ...
    else
    {
        ...
        sprite.fogdensity = gld_CalcFogDensity(
            thing->subsector->sector, lightlevel, GLDIT_SPRITE
        );
    }
    sprite.cm =
...
{
    // e6y
    // The ultimate 'ATI sucks' fix: Some of ATIs graphics cards are so
    // unprecise when rendering geometry that each and every border between
    // polygons must be seamless, otherwise there are rendering artifacts worse
    // than anything that could be seen on Geforce 2's! Made this a menu option
    // because the speed impact is quite severe and this special handling is not
    // necessary on modern NVidia cards.
    seg_t *seg = wall->seg;
...
static void gld_DrawItemsSortByTexture(GLDrawItemType itemtype)
{
    ...
    {
        qsort(
            gld_drawinfo.items[itemtype], gld_drawinfo.num_items[itemtype],
...
static void gld_DrawItemsSort(
    ...
    int(C_DECL *PtFuncCompare)(const void *, const void *)
)
{
    qsort(
        gld_drawinfo.items[itemtype], gld_drawinfo.num_items[itemtype],
...
static void gld_DrawItemsSortSprites(GLDrawItemType itemtype)
{
    ...
    if (sprites_doom_order == DOOM_ORDER_DYNAMIC)
    {
        ...
        if (!no_overlapped_sprites)
        {
            ...
            {
                GLSprite *sprite1 =
                    gld_drawinfo.items[itemtype][i - 1].item.sprite;
                GLSprite *sprite2 =
                    gld_drawinfo.items[itemtype][i - 0].item.sprite;
                ...
                if (sprite1->xy == sprite2->xy)
                {
                    ...
                    i++;
                    while (i < count &&
                           gld_drawinfo.items[itemtype][i].item.sprite->xy ==
                               sprite1->xy)
                    {
                        ...
                        {
                            sprite =
                                gld_drawinfo.items[itemtype][i].item.sprite;
                        }
                        i++;
                    }
...
void gld_InitDisplayLists()
{
    ...
    if (gl_use_display_lists)
    {
        ...
        if (details_count && gl_arb_multitexture)
        {
            ...
            flats_detail_display_list =
                glGenLists(flats_detail_display_list_size);
...
void gld_CleanDisplayLists()
{
    if (gl_use_display_lists)
    {
        ...
        {
            glDeleteLists(
                flats_detail_display_list, flats_detail_display_list_size
            );
...
void gld_DrawScene(player_t *player)
{
    ...
    if (gl_blend_animations)
    {
        ...
        {
            gld_SetFog(
                gld_drawinfo.items[GLDIT_ACEILING][i].item.flat->fogdensity
            );
        ...
        gld_drawinfo.num_items[GLDIT_TSPRITE] > 0)
    {
        if (gld_drawinfo.num_items[GLDIT_TWALL] > 0)
        {
            ...
            {
                gld_SetFog(
                    gld_drawinfo.items[GLDIT_TWALL][i].item.wall->fogdensity
                );
