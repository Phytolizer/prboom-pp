...
static const char *yah[3] = {"WIURH0", "WIURH1", nullptr};

// splat
static const char *splat[2] = {"WISPLAT", nullptr};
...
static void WI_drawTime(int x, int y, int t)
{
    ...
    if (t < 100 * 60 * 60)
    {
        for (;;)
        {
            n = t % 60;
            t /= 60;
            x = WI_drawNum(x, y, n, (t || n > 9) ? 2 : 1) -
                V_NamePatchWidth(colon);
...
void WI_updateDeathmatchStats(void)
{
    ...
    if (acceleratestage && dm_state != 4) // still ticking
    {
        ...
        S_StartSound(nullptr, sfx_barexp); // bang
    ...
    if (dm_state == 2)
    {
        if (!(bcnt & 3))
        {
            S_StartSound(nullptr, sfx_pistol); // noise while counting
        }
            ...
            if (playeringame[i])
            {
                for (j = 0; j < MAXPLAYERS; j++)
                {
                    if (playeringame[j] && dm_frags[i][j] != plrs[i].frags[j])
                    {
                        ...
                        if (dm_frags[i][j] > 999)
                        { // Ty 03/17/98 3-digit frag count
                            dm_frags[i][j] = 999;
                        }
...
void WI_updateNetgameStats(void)
{
    ...
    if (ng_state == 2)
    {
        if (!(bcnt & 3))
        {
            S_StartSound(nullptr, sfx_pistol); // pop
        }
...
void WI_updateStats(void)
{
    ...
    else if (sp_state == 6)
    {
        ...
        {
            cnt_secret[0] =
                (wbs->maxsecret ? (plrs[me].ssecret * 100) / wbs->maxsecret
                                : 100);
            S_StartSound(nullptr, sfx_barexp);
            sp_state++;
        }
