...
int EV_VerticalDoor(line_t *line, mobj_t *thing)
{
    ...
    // heretic_note: I don't see where the return value is used...keeping
    // heretic signature
    if (heretic)
    {
                 ...
                 (line->special == 28)))
    {
            ...
            door->thinker.function == T_VerticalDoor)
        {
            ...
            /* Write this to the thinker. In demo compatibility mode, we might
             * be overwriting a field of a non-vldoor_t thinker - we need to add
             * any other thinker types here if any demos depend on specific
             * fields being corrupted by this.
             */
            if (outval)
            {
                if (door->thinker.function == T_VerticalDoor)
