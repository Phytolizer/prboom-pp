...
void GetFirstMap(
    int *ep,
    int *map
); // Ty 08/29/98 - add "-warp x" functionality
    ...
    if (doSkip)
    {
        ...
        else
        {
            ...
            if (!dsda_InputActivated(dsda_input_use) &&
                ...
                (ev->type == ev_keydown || ev->type == ev_keyup
...
void D_Display(fixed_t frac)
{
    ...
    if (paused && (menuactive != mnact_full))
    {
        ...
        {
            // Simplified the "logic" here and no need for x-coord caching -
            // POPE
            V_DrawNamePatch(
                (320 - V_NamePatchWidth("M_PAUSE")) / 2, 4, 0, "M_PAUSE",
...
static void D_DoomLoop()
{
    ...
    for (;;)
    {
        ...
        if (avi_shot_fname && !doSkip)
        {
            ...
            avi_shot_num++;
            len = snprintf(
                nullptr, 0, "%s%06d.tga", avi_shot_fname, avi_shot_num
            );
            avi_shot_curr_fname = static_cast<char *>(malloc(len + 1));
            sprintf(
                avi_shot_curr_fname, "%s%06d.tga", avi_shot_fname, avi_shot_num
            );
    ...
    if (!access(iwadname, R_OK))
    {
        ...
        if ((fp = fopen(iwadname, "rb")))
        {
            ...
            if (fread(&header, sizeof(header), 1, fp) == 1)
            {
                ...
                if (strncmp(
                        header.identification, "IWAD",
                        4
                    )) // missing IWAD tag in header
                {
                    noiwad = true;
                }
...
void AddIWAD(const char *iwad)
{
    ...
    lprintf(
        LO_CONFIRM, "IWAD found: %s\n",
        iwad
    ); // jff 4/20/98 print only if found
...
static void IdentifyVersion()
{
        ...
        i < myargc - 1) // jff 3/24/98 if -save present
    {
        ...
        {
            lprintf(
                LO_ERROR, "Error: -save path does not exist, using %s\n",
                basesavegame
            );
        }
    }
...
static void FindResponseFile()
{
    ...
    for (i = 1; i < myargc; i++)
    {
        if (myargv[i][0] == '@')
        {
            ...
            byte *file = nullptr;
            const char **moreargs =
                static_cast<const char **>(malloc(myargc * sizeof(const char *))
                );
            char **newargv;
            ...
            if (size < 0)
            {
                size_t fnlen = doom_snprintf(
                    nullptr, 0, "%s/%s", I_DoomExeDir(), &myargv[i][1]
                );
                fname = static_cast<char *>(realloc(fname, fnlen + 4 + 1));
                doom_snprintf(
                    fname, fnlen + 1, "%s/%s", I_DoomExeDir(), &myargv[i][1]
                );
                AddDefaultExtension(fname, ".rsp");
                size = M_ReadFile(fname, &file);
            }
            ...
            memcpy(
                (void *)&newargv[indexinfile], moreargs,
...
static void DoLooseFiles()
{
    ...
    if (wadcount + lmpcount + dehcount != 0)
    {
        ...
        while (params[k].cmdparam)
        {
            ...
            {
                tmyargv[tmyargc++] =
                    strdup(params[k].cmdparam); // put the switch in
...
static void L_SetupConsoleMasks()
{
    ...
    const char *cena = "ICWEFDA",
               *pos; // jff 9/3/98 use this for parsing console masks // CPhipps
                     // - const char*'s
    ...
    if ((p = M_CheckParm("-cout")))
    {
        ...
        {
            for (i = 0, cons_output_mask = 0; (size_t)i < strlen(myargv[p]);
                 i++)
            {
...
static void D_DoomMainSetup()
{
...

    {
        ...
        lprintf(
            ...
            "It comes with ABSOLUTELY NO WARRANTY. See the file COPYING "
            "for details.\n",
    ...
    if ((p = M_CheckParm("-timer")) && p < myargc - 1 && deathmatch)
    {
        ...
        // jff 9/3/98 use logical output routine
        lprintf(
            LO_CONFIRM, "Levels will end after %d minute%s.\n", time,
            time > 1 ? "s" : ""
        );
    }
    ...
    {
        startmap =
            0; // Ty 08/29/98 - allow "-warp x" to go to first map in wad(s)
        ...
        if (gamemode == commercial)
        {
            ...
            {
                startmap =
                    atoi(myargv[p + 1]); // Ty 08/29/98 - add test if last parm
            }
        }
    ...
    if (p && p < myargc - 1)
    {
        ...
        viewangleoffset = viewangleoffset < 0
                              ? 0
                              : (viewangleoffset > 7 ? 7 : viewangleoffset);
    ...
    {
        // only autoloaded wads here - autoloaded patches moved down below
        // W_Init
        int i;
    ...
    { /* killough */
        ...
        else
        {
            if ((p = IsDemoContinue()))
            {
                democontinue = true;
                AddDefaultExtension(
                    strcpy(democontinuename, myargv[p + 2]), ".lmp"
                );
            }
            else
            {
    ...
    if (p && p < myargc - 1)
    {
        char *file = static_cast<char *>(malloc(strlen(myargv[p + 1]) + 4 + 1)
        ); // cph - localised
    ...
    {
        // MBF-style DeHackEd in wad support: load all lumps, not just the last
        // one
        for (p = -1; (p = W_ListNumFromName("DEHACKED", p)) >= 0;)
        {
                ...
                lumpinfo[p].source == source_auto_load)
            {
                ProcessDehFile(
                    nullptr, D_dehout(),
                    p
                ); // cph - add dehacked-in-a-wad support
            }
        }
    ...
    if (!M_CheckParm("-noload"))
    {
        // now do autoloaded dehacked patches, after IWAD patches but before
        // PWAD
        int i;
    ...
    if (p)
    {
        ...
        while (++p != myargc && *myargv[p] != '-')
        {
            ...
            {
                I_Error(
                    "D_DoomMainSetup: Cannot find .deh or .bex file named %s",
                    myargv[p]
                );
            }
        }
    ...
    if (!M_CheckParm("-nomapinfo"))
    {
        int lump_index;
        for (lump_index = -1;
             (lump_index = W_ListNumFromName("UMAPINFO", lump_index)) >= 0;)
        {
            const unsigned char *lump =
                (const unsigned char *)W_CacheLumpNum(lump_index);
    ...
    if (slot && ++slot < myargc)
    {
        ...
        G_LoadGame(
            slot,
            true
        ); // killough 5/15/98: add command flag // cph - no filename
    }
    else if (!singledemo)
    { /* killough 12/98 */
...
void GetFirstMap(int *ep, int *map)
{
    ...
    if (*map == 0)    // unknown so go search for first changed one
    {
        ...
        *map = 1; // default E1M1 or MAP01
        if (gamemode == commercial)
        {
            for (i = 1; !done && i < 33; i++) // Ty 09/13/98 - add use of !done
            {
                ...
                if (ix != -1) // Ty 10/04/98 avoid -1 subscript
                {
                    ...
                    {
                        if (!*name)
                        { // found one, not pwad.  First default.
                            strcpy(name, test);
                        }
                    }
                }
        ...
        // jff 9/3/98 use logical output routine
        lprintf(
            LO_CONFIRM, "Auto-warping to first %slevel: %s\n",
            newlevel ? "new " : "", name
