...
void gld_GetTextureTexID(GLTexture *gltexture, int cm)
{
    ...
    gltexture->texid_p =
        &gltexture->glTexExID[cm][gltexture->player_cm][boom_cm];
    return;
}
...
static GLTexture *gld_AddNewGLTexItem(int num, int count, GLTexture ***items)
{
    ...
    {
        (*items) =
            static_cast<GLTexture **>(std::calloc(count, sizeof(GLTexture *)));
    }
    if (!(*items)[num])
    {
        (*items)[num] =
            static_cast<GLTexture *>(std::calloc(1, sizeof(GLTexture)));
                ...
                for (j = js; j < je; j++, pos += (4 * gltexture->buffer_width))
                {
                    ...
                    if (gl_boom_colormaps && use_boom_cm)
                    {
                        const lighttable_t *colormap =
                            (fixedcolormap ? fixedcolormap : fullcolormap);
                        buffer[pos + 0] =
                            playpal[colormap[outr[source[j]]] * 3 + 0];
                        buffer[pos + 1] =
                            playpal[colormap[outr[source[j]]] * 3 + 1];
                        buffer[pos + 2] =
                            playpal[colormap[outr[source[j]]] * 3 + 2];
                    }
                    else
                    {
            ...
            for (x = 0; x < gltexture->realtexwidth; x++, pos += 4)
            {
                ...
                if (gl_boom_colormaps && use_boom_cm)
                {
                    const lighttable_t *colormap =
                        (fixedcolormap ? fixedcolormap : fullcolormap);
                    buffer[pos + 0] =
                        playpal[colormap[flat[y * 64 + x]] * 3 + 0];
                    buffer[pos + 1] =
                        playpal[colormap[flat[y * 64 + x]] * 3 + 1];
                    buffer[pos + 2] =
                        playpal[colormap[flat[y * 64 + x]] * 3 + 2];
                }
                else
                {
...
GLTexture *gld_RegisterTexture(int texture_num, dboolean mipmap, dboolean force)
{
    ...
    if (gltexture->textype == GLDT_UNREGISTERED)
    {
        ...
        gltexture->height =
            MIN(gltexture->realtexheight, gltexture->tex_height);
...
{
    // if ((gltexture->flags & GLTEXTURE_CLAMPXY) != (flags &
    // GLTEXTURE_CLAMPXY))
        ...
        for (k = 0; k < 3; k++)
        {
            if (textureanims[k] && textureanims[k]->istexture)
            {
                for (j = textureanims[k]->basepic;
                     j < textureanims[k]->basepic + textureanims[k]->numpics;
                     j++)
                {
                    hitlist[j] = 1;
                }
