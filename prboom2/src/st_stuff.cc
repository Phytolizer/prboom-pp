...
static void ST_refreshBackground()
{
    ...
    if (st_statusbaron)
    {
        ...
        if (!deathmatch)
        {
            V_DrawNumPatch(ST_ARMSBGX, y, BG, armsbg.lumpnum, CR_DEFAULT,
                           flags);
        }
    ...
    if (priority < 8)
    {
        if (plyr->damagecount && plyr->attacker && plyr->attacker != plyr->mo)
        {
                     ...
                     : (st_oldhealth - plyr->health)) > ST_MUCHPAIN)
            {
                ...
                // Not only was the condition reversed, but the priority system
                // is broken in a way that makes the face not work with monster
                // damage.
                if (!comp[comp_ouchface])
                {
                    priority = 8;
...
static void ST_doPaletteStuff()
{
    ...
    if (palette != st_palette)
    {
        V_SetPalette(st_palette =
                         palette); // CPhipps - use new palette function
...
static void ST_drawWidgets(dboolean refresh)
{
    ...
    for (i = 0; i < 4; i++)
    {
        STlib_updateNum(&w_ammo[i], CR_DEFAULT,
                        refresh); // jff 2/16/98 no xlation
    ...
    else
    {
        ...
        {
            STlib_updatePercent(&w_armor, CR_BLUE2, refresh); // killough
        }
        // 2/28/98
    }
...
void ST_Drawer(dboolean statusbaron, dboolean refresh, dboolean fullmenu)
{
    ...
    if (statusbaron)
    {
        if (st_firsttime || (V_GetMode() == VID_MODEGL))
        {
            ...
            st_firsttime = false;
            ST_refreshBackground(); // draw status bar background to off-screen
                                    // buff
            if (!fullmenu)
            {
