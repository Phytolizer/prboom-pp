...
angle_t R_PointToAngleEx2(fixed_t x1, fixed_t y1, fixed_t x, fixed_t y)
{
    ...
    if (x_viewx < INT_MIN || x_viewx > INT_MAX || y_viewy < INT_MIN ||
        y_viewy > INT_MAX)
    {
        // [crispy] preserving the angle by halfing the distance in both
        // directions
        x = (int)(x_viewx / 2 + x1);
        y = (int)(y_viewy / 2 + y1);
    }
...
{
    // Note: float won't work here as it's less precise than the BAM values
...
static void R_InitLightTables()
{
    ...
    {
        // SoM: the LIGHTBRIGHT constant must be used to scale the start offset
        // of the colormaps, otherwise the levels are staggered and become
        // slightly darker.
...
void R_SetupViewScaling()
{
    ...
    // SoM: ok, assemble the realx1/x2 arrays differently. To start, we are
    // using floats to do the scaling which is 100 times more accurate,
    // secondly, I realized that the reason the old single arrays were causing
    // problems was they was only calculating the top-left corner of the scaled
    // pixels. Calculating widths through these arrays is wrong because the
    // scaling will change the final scaled widths depending on what their
    // unscaled screen coords were. Thusly, all rectangles should be converted
    // to unscaled x1, y1, x2, y2 coords, scaled, and then converted back to x,
    // y, w, h
    //
    // e6y: wide-res
...
void R_SetupFreelook()
{
    if (V_GetMode() != VID_MODEGL)
    {
        ...
        globaldclip =
            FixedDiv((viewheight << FRACBITS) - centeryfrac, InvZtoScale);
