...
void S_ChangeMusic(int musicnum, int looping)
{
    ...
    if (!music->lumpnum)
    {
        ...
        // HERETIC_TODO: put d_ where it belongs in the definition and remove
        // this
...
)
{
    ...
    // which is not displayplayer and displayplayer was not spawned at the
    // moment. It happens in multiplayer demos only.
    //
    // Stack trace is:
...
static void Heretic_S_StartSound(void *_origin, int sound_id)
{
    ...
    channels[i].pitch =
        (byte)(NORM_PITCH + (M_Random() & 7) - (M_Random() & 7));
    channels[i].handle =
