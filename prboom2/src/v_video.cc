    ...
    if (!flexTranInit)
    {
        ...
        for (r = 0; r < 32; r++)
        {
            for (g = 0; g < 32; g++)
            {
                for (b = 0; b < 32; b++)
                {
                    RGB32k[r][g][b] = V_BestColor(palette, MAKECOLOR(r),
            ...
            for (p = 0; p < numPals; p++)
            {
                for (i = 0; i < 256; i++)
                {
                    ...
                    for (w = 0; w < VID_NUMCOLORWEIGHTS; w++)
                    {
                        ...
                        nb = (int)(b * t + roundUpB);
                        playpal_data
                            ->Palettes32[((p * 256 + i) * VID_NUMCOLORWEIGHTS) +
                                         w] = ((nr << 16) | (ng << 8) | nb);
                    }
                }
            }
            ...
            for (p = 0; p < numPals; p++)
            {
                for (i = 0; i < 256; i++)
                {
                    ...
                    for (w = 0; w < VID_NUMCOLORWEIGHTS; w++)
                    {
                        ...
                        playpal_data
                            ->Palettes16[((p * 256 + i) * VID_NUMCOLORWEIGHTS) +
                                         w] = ((nr << 11) | (ng << 5) | nb);
                    }
                }
            }
            ...
            for (p = 0; p < numPals; p++)
            {
                for (i = 0; i < 256; i++)
                {
                    ...
                    for (w = 0; w < VID_NUMCOLORWEIGHTS; w++)
                    {
                        ...
                        nb = (int)((b >> 3) * t + roundUpB);
                        playpal_data
                            ->Palettes15[((p * 256 + i) * VID_NUMCOLORWEIGHTS) +
                                         w] = ((nr << 10) | (ng << 5) | nb);
                    }
                }
            }
    ...
    if (dy > dx)
    {
        ...
        while (--dy)
        {
            ...
            V_PlotPixelWu(0, x, y, (byte)color,
                          finecosine[erroracc >> wu_fineshift] >>
                              wu_fixedshift);
...
void V_FillBorder(int lump, byte color)
{
    ...
    if (lump >= 0)
    {
        if (bordtop > 0)
        {
            ...
            // Bottom
            V_FillFlat(lump, 0, 0, SCREENHEIGHT - bordbottom, SCREENWIDTH,
                       bordbottom, VPT_NONE);
        }
        if (bordleft > 0)
        {
    ...
    else
    {
        ...
        {
            // Left
            V_FillRect(0, 0, bordtop, bordleft,
