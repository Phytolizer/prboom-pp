...
static dboolean R_CheckBBox(const fixed_t *bspcoord)
{
    ...
    angle1 =
        R_PointToAngleEx(bspcoord[check[0]], bspcoord[check[1]]) - viewangle;
    angle2 =
        R_PointToAngleEx(bspcoord[check[2]], bspcoord[check[3]]) - viewangle;
...
static void R_Subsector(int num)
{
    ...
    // memory is recycled, compiler optimizations will make the floorplane
    // pointer a dangling pointer when passed into gld_AddPlane().
    ...
    if (V_GetMode() == VID_MODEGL)
    {
        ...
        if (!gl_use_stencil && frontsector == sub->sector)
        {
            ...
            // if the sector has bottomtextures, then the floorheight will be
            // set to the highest surounding floorheight
            ...
            // the same for ceilings. they will be set to the lowest
            // ceilingheight
            if ((frontsector->flags & NO_TOPTEXTURES) || (!ceilingplane))
            {
                ...
                if (tmpsec &&
                    frontsector->ceilingheight != tmpsec->ceilingheight)
                {
