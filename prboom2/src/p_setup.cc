...
static void P_LoadSegs(int lump)
{
    ...
    for (i = 0; i < numsegs; i++)
    {
        ...
        // li->length  = GetDistance(li->v2->x - li->v1->x, li->v2->y -
        // li->v1->y);
        ...
        {
            char str[200] = "P_LoadSegs: compatibility loss - seg %d "
...
static void P_LoadSegs_V4(int lump)
{
    ...
    for (i = 0; i < numsegs; i++)
    {
        ...
        if (v1 >= numvertexes || v2 >= numvertexes)
        {
            char str[200] = "P_LoadSegs_V4: compatibility loss - seg %d "
                            "references a non-existent vertex %d\n";
...
static void P_LoadZNodes(int lump, int glnodes, int compressed)
{
    ...
    if (!samelevel)
    {
        ...
        for (i = 0; i < newVerts; i++)
        {
            newvertarray[i + orgVerts].x =
                LittleLong(*((const unsigned int *)data));
    ...
    // Subsequent subsectors starts with the next unused segment number
    // (currSeg)
...
static void P_LoadThings(int lump)
{
    ...
    const mapthing_t *data =
        static_cast<const mapthing_t *>(W_CacheLumpNum(lump));
...
static void P_LoadLineDefs(int lump)
{
    ...
    for (i = 0; i < numlines; i++)
    {
...

        {
             ...
             * cph 2002/07/20 - these errors are fatal if not fixed, so apply
            ...
            if ((ld->sidenum[1] == NO_INDEX) && (ld->flags & ML_TWOSIDED))
            {
                ...
                // purposes see CLNJ-506.LMP at http://doomedsda.us/wad1005.html
                MissedBackSideOverrun(ld);
                if (!demo_compatibility || !EMULATE(OVERFLOW_MISSEDBACKSIDE))
                {
                    ld->flags &=
                        ~ML_TWOSIDED; // Clear 2s flag for missing left side
                }
...
static int P_GroupLines(void)
{
    ...
    { // allocate line tables for each sector
        line_t **linebuffer =
            static_cast<line_t **>(std::malloc(total * sizeof(line_t *)));
        ...
        if (l->dx && l->dy)  // We can ignore orthogonal lines
        {
            ...
            do
            {
                if (!hit[v - vertexes]) // If we haven't processed vertex
                {
                    ...
                    if (v != l->v1 &&
                        v != l->v2) // Exclude endpoints of linedefs
                    { // Project the vertex back onto the parent linedef
                        int_64_t dx2 =
                            (l->dx >> FRACBITS) * (l->dx >> FRACBITS);
                        int_64_t dy2 =
                            (l->dy >> FRACBITS) * (l->dy >> FRACBITS);
                        int_64_t dxy =
                            (l->dx >> FRACBITS) * (l->dy >> FRACBITS);
                        int_64_t s = dx2 + dy2;
                        ...
                        // [crispy] wait a minute... moved more than 8 map
                        // units? maybe that's a linguortal then, back to the
