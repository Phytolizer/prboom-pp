...
{
  int result = CheckForIdentifier(lumpnum + ML_NODES, reinterpret_cast<const byte *>("xNd4\0\0\0\0"), 8);
...
static void P_LoadSegs (int lump)
{
  ...
  segs = static_cast<seg_t *>(calloc_IfSameLevel(segs, numsegs, sizeof(seg_t)));
...
static void P_LoadZNodes(int lump, int glnodes, int compressed)
{
  ...
  if (!samelevel)
  {
    ...
    {
      newvertarray = calloc<vertex_t *>(orgVerts + newVerts, sizeof(vertex_t));
...
void P_InitSubsectorsLines(void)
{
  ...
  count = 0;
  sslines_indexes = malloc<int *>((numsubsectors + 1) * sizeof(sslines_indexes[0]));
