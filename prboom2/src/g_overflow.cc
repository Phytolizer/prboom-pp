                                ...
                                const char *params, ...)
{
    ...
    if (overflows[overflow].warn && !overflows[overflow].promted)
    {
        ...
        static const char str3[] = "%s overflow has been detected. "
                                   ...
                                   "in case you're recording demo.%s%s";
    ...
    if (demo_compatibility && numspechit > 8)
    {
        ...
        if (EMULATE(OVERFLOW_SPECHIT))
        {
                ...
                compatibility_level == tasdoom_compatibility)
            {
                ...
                // http://www.doomworld.com/sda/flsofdth.htm hr.wad\hf181430.lmp
...
void RejectOverrun(int rejectlump, const byte **rejectmatrix, int totallines)
{
    ...
    if (length < required)
    {
        ...
        *rejectmatrix = static_cast<const byte *>(
        ...
        // I don't think there are any demos that will need it but yes that
        // seems sensible
        ...
        if (demo_compatibility && PROCESS(OVERFLOW_REJECT))
        {
            ...
            if (EMULATE(OVERFLOW_REJECT))
            {
                ...
                rejectpad[0] = ((totallines * 4 + 3) & ~3) +
                               24; // doom.exe zone header size

                // copy at most 16 bytes from rejectpad
                // emulating a 32-bit, little-endian architecture (can't
                // memmove)
...
static int GetMemoryValue(unsigned int offset, void *value, int size)
{
    ...
    if (value)
    {
        ...
        case 1:
            *((unsigned char *)value) =
                *((unsigned char *)(&dos_mem_dump[offset]));
            return true;
        case 2:
            *((unsigned short *)value) =
                *((unsigned short *)(&dos_mem_dump[offset]));
            return true;
        case 4:
            *((unsigned int *)value) =
                *((unsigned int *)(&dos_mem_dump[offset]));
            return true;
        }
    }
