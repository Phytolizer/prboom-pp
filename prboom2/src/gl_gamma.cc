...
int gld_SetGammaRamp(int gamma)
{
    ...
    else
    {
        if (first && desired_fullscreen)
        {
            ...
            // reset, it resets the gamma ramp, but the driver apparently keeps
            // a cached copy of the ramp that it doesn't update, so when
            ...
            //
            // So here we force the gamma ramp to something absolutely horrible
            // and trust that we will be able to properly set the gamma later
            first = false;
            memset(gammatable, 0, sizeof(gammatable));
            SDL_SetWindowGammaRamp(sdl_window, nullptr, NULL, gammatable);
        }
