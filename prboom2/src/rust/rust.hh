...
struct EnemyStats {
  ...
  EnemyStats(mobjtype_t const& enemy,
             unsigned long const& kills)
    : enemy(enemy),
      kills(kills)
  {}
...
struct RawWeaponStats {
  ...
  RawWeaponStats(unsigned long const& kills,
                 EnemyStats *const& enemies,
...
struct SoundInfo {
  ...
  SoundInfo(sfxenum_t const& sfx,
            const uint8_t *const *const& lumps,
            const size_t *const& lump_lens,
            size_t const& num_lumps,
            bool const& singularity,
            int const& pitch,
            int const& volume)
    : sfx(sfx),
      lumps(lumps),
  ...
  bool operator==(const SoundInfo& other) const {
    return sfx == other.sfx &&
           lumps == other.lumps &&
           lump_lens == other.lump_lens &&
           num_lumps == other.num_lumps &&
           singularity == other.singularity &&
           pitch == other.pitch &&
           volume == other.volume;
  }
  bool operator!=(const SoundInfo& other) const {
