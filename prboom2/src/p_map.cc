...
dboolean P_TeleportMove(mobj_t *thing, fixed_t x, fixed_t y, dboolean boss)
{
    ...
    /* killough 8/9/98: make telefragging more consistent, preserve
     * compatibility */
...
static int untouched(line_t *ld)
{
    ...
    return (tm_bounding_box[BOXRIGHT] = (x = tmthing->x) + tmthing->radius) <=
               ld->bbox[BOXLEFT] ||
           (tm_bounding_box[BOXLEFT] = x - tmthing->radius) >=
               ld->bbox[BOXRIGHT] ||
           (tm_bounding_box[BOXTOP] = (y = tmthing->y) + tmthing->radius) <=
               ld->bbox[BOXBOTTOM] ||
           (tm_bounding_box[BOXBOTTOM] = y - tmthing->radius) >=
               ld->bbox[BOXTOP] ||
    ...
    PIT_CheckLine(line_t *ld)
{
    ...
    // killough 7/24/98: allow player to move out of 1s wall, to prevent
    // sticking
    ...
    if (!(tmthing->flags & (MF_MISSILE | MF_BOUNCES)))
    {
        ...
        if (!(tmthing->flags & MF_FRIEND || tmthing->player) &&
            ld->flags & ML_BLOCKMONSTERS && tmthing->type != HERETIC_MT_POD)
...
static dboolean PIT_CheckThing(mobj_t *thing) // killough 3/26/98: make static
{
    ...
    // should not be moved up to first, as it adds more overhead than it
    // removes.
    ...
    if (tmthing->flags & MF_SKULLFLY)
    {
        ...
        int new_state;
        int skull_damage =
            ((P_Random(pr_skullfly) % 8) + 1) * tmthing->info->damage;
    ...
    if (!(thing->flags & MF_NOCLIP))
    {
        ...
        if (!(thing->flags & (MF_DROPOFF | MF_FLOAT)))
        {
            if (comp[comp_dropoff])
            {
                ...
                // -force_no_dropoff command-line switch is for
                // mbf_compatibility demos recorded with prboom 2.2.2 - 2.4.7
                // Links:
...
void P_ApplyTorque(mobj_t *mo)
{
    ...
    int xh =
        P_GetSafeBlockX((tmbbox[BOXRIGHT] = mo->x + mo->radius) - bmaporgx);
    int yl =
        P_GetSafeBlockY((tmbbox[BOXBOTTOM] = mo->y - mo->radius) - bmaporgy);
...
void P_SlideMove(mobj_t *mo)
{
    ...
    do
    {
        ...
        /* killough 10/98: affect the bobbing the same way (but not voodoo
         * dolls) cph - DEMOSYNC? */
...
dboolean PTR_ShootTraverse(intercept_t *in)
{
    ...
    if (in->isaline)
    {
        ...
        { // crosses a two sided (really 2s) line
            ...
            else if ((li->frontsector->floorheight ==
...
dboolean P_CheckSector(sector_t *sector, dboolean crunch)
{
    ...
    do
        for (n = sector->touching_thinglist; n;
             n = n->m_snext) // go through list
            if (!n->visited) // unprocessed thing found
            {
                n->visited = true; // mark thing as processed
                if (!(n->m_thing->flags &
                      MF_NOBLOCKMAP))             // jff 4/7/98 don't do these
