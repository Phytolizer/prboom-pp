...
dboolean D_NetGetWad(const char *name)
{
    ...
    if (!strcasecmp((void *)(packet + 1), name))
    {
         ...
         * URLs) Unix systems have all these commands handy, this kind of thing
         * is easy Any windo$e port will have some awkward work replacing these.
         */
    ...
    { // Receive network packets
        ...
        while ((recvlen = I_GetPacket(packet, 10000)))
        {
            ...
            case PKT_DOWN: // Server downed
            {
                ...
                server = false;
                doom_printf("Server is down\nAll other players are no longer "
                            "in the game\n");
            }
            break;
