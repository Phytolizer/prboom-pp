    ...
    if (heretic)
    {
            ...
            !players[consoleplayer].powers[pw_weaponlevel2])
        {
            cmd->arti = arti_tomeofpower;
                 ...
                 !cmd->arti && (players[consoleplayer].mo->health < MAXHEALTH))
        {
    ...
    { // phares 02/26/98: Added gamemode checks
        ...
        else
        {
            ...
            newweapon =
                ...
                : (!demo_compatibility &&
                   dsda_InputActive(dsda_input_weapon9) &&
                   gamemode == commercial)
                    ? wp_supershotgun
                    : wp_nochange;
        }
...
static void G_DoLoadLevel(void)
{
    ...
    if (!demoplayback)
    {                                  // Don't switch views if playing a demo
...
dboolean G_Responder(event_t *ev)
{
    ...
    case ev_mousemotion: {
        ...
        {
            value =
                (double)mouseSensitivity_vert * AccelerateMouse(ev->data3) / 8;
...
void G_Ticker(void)
{
        ...
        for (i = 0; i < MAXPLAYERS; i++)
        {
            if (playeringame[i])
            {
                ...
                if ((netgame || demoplayback) &&
                    cmd->forwardmove > TURBOTHRESHOLD && !(gametic & 31) &&
                    ((gametic >> 5) & 3) == i)
                {
                ...
                if (netgame && !netdemo && !(gametic % ticdup))
                {
                    ...
                    {
                        consistancy[i][buf] =
                            static_cast<short>(players[i].mo->x);
                    }
                    else
                    {
    ...
    if (gamestate != prevgamestate)
    {
        ...
        case GS_LEVEL:
            ...
            // crash wowever, with this fix in, the exit switch sound is cut off
            // S_Stop();
...
static dboolean G_CheckSpot(int playernum, mapthing_t *mthing)
{
    ...
    { // Teleport fog at respawn point
         ...
         * We have to emulate original Doom's behaviour, deferencing past the
         * start of the array, into the previous array (finetangent) */
...
void G_DoCompleted(void)
{
    ...
    if (gamemode != commercial) // kilough 2/7/98
    {
        ...
        else
        {
            switch (gamemap)
            {
            // cph - Remove ExM8 special case, so it gets summary screen
            // displayed
            case 9:
                for (i = 0; i < MAXPLAYERS; i++)
                {
    ...
    {
        if (gameepisode >= 1 && gameepisode <= 4 && gamemap >= 1 &&
            gamemap <= 9)
        {
...
frommapinfo:
     ...
     * totalleveltimes value; so our total is compatible with the "naive" total
     * of just adding the times in seconds shown for each level. Also means our
     * total time will agree with Compet-n.
     */
...
static byte map_old_comp_levels[] = {0,  1,  2,  4,  5,  7,  8, 9,
...
void G_DoLoadGame(void)
{
    ...
    {
        gameaction =
            ga_loadgame; /* Mark that we're loading a game before demo */
...
void G_Compatibility(void)
{
    static const struct
    {
        ...
        // comp_telefrag - monsters used to telefrag only on MAP30, now they do
        // it for spawners only
        ...
        // comp_pain - original Doom limits Pain Elementals from spawning too
        // many skulls
        {boom_compatibility, mbf_compatibility},
        ...
        // comp_respawn - objects which aren't on the map at game start respawn
        // at (0,0)
...
struct MapEntry *G_LookupMapinfo(int game_episode, int game_map)
{
    ...
    if (gamemode == commercial)
    {
        snprintf(lumpname, 9, "MAP%02d", game_map);
    }
    else
    {
        snprintf(lumpname, 9, "E%dM%d", game_episode, game_map);
    }
                    ...
                    // definitions. They do not make sense in this case.
    {
        ...
        else
        {
            ...
            else if (gamemode == retail)
            {
                ...
                // switch E5M1 from 2002ado.wad is an example. Now you can play
                // it with "-warp 5 1 -complevel 3". 'Vanilla' Ultimate Doom
                // executable also allows it.
...
byte *G_WriteOptions(byte *demo_pointer)
{
    ...
    *demo_pointer++ =
        static_cast<byte>(monsters_remember); // part of monster AI
    ...
    *demo_pointer++ =
        static_cast<byte>(monster_avoid_hazards); // killough 9/9/98
    ...
    *demo_pointer++ = (compatibility_level >= prboom_2_compatibility) &&
        ...
        *demo_pointer++ = (num_extensions >> 8) & 0xff;
        ...
        if (umapinfo_loaded)
        {
            ...
            *demo_pointer++ = 'O';
            ...
            // we wouldn't just plop this here, but right now we only support
            // the 1 in the future, we should assume that chunks in the header
            // should follow the order of their appearance in the extensions
            // table.
            if (mapname_len > 8)
            {
            ...
            {
                // FIXME - the toupper is a hacky workaround for the case
                // insensitivity in the current UMAPINFO reader. lump names
                // should probably not be lowercase ever (?)
    ...
    if (mbf_features)
    {
        { /* Write version code into demo */
            ...
            case prboom_6_compatibility:
                ...
                break;
            default:
                I_Error("G_BeginRecording: PrBoom compatibility level "
                        "unrecognised?");
            }
            *demo_pointer++ = v;
        }
        ...
        *demo_pointer++ = static_cast<byte>(consoleplayer);
        ...
        demo_pointer =
            G_WriteOptions(demo_pointer); // killough 3/1/98: Save game options
                               ...
                               unsigned int params)
{
    ...
    if (demover == 255)
    {
        ...
        {
            I_Error("G_ReadDemoHeader: Extended demo format 255 found, but "
                    "\"PR+UM\" string not found.");
        }
        ...
        for (i = 0; i < num_extensions; i++)
        {
            ...
            if (r_len == 8 &&
                strncmp((const char *)demo_pointer, "UMAPINFO", 8) == 0)
            {
                using_umapinfo = 1;
            }
            else
            {
                ...
                I_Error("G_ReadDemoHeader: Extended demo format extension "
                        "unrecognized.");
            }
            ...
            if (strncmp((const char *)demo_pointer, "MAP", 3) == 0)
            {
                ...
                for (i = 0; demo_pointer < string_end; i++)
                {
                    ...
                    if (cur < '0' || cur > '9')
                    {
                        if (cur != 0)
                        {
                            I_Error("G_ReadDemoHeader: Unable to determine map "
                                    "for UMAPINFO demo.");
                        }
                        break;
                    }
            ...
            else if (*demo_pointer++ == 'E')
            {
                ...
                for (i = 0; demo_pointer < string_end; i++)
                {
                    ...
                    if (cur < '0' || cur > '9')
                    {
                        ...
                        I_Error("G_ReadDemoHeader: Unable to determine map for "
                                "UMAPINFO demo.");
                    }
    ...
    // ano - okay we are done with most of the 255 extension code past this
    // point demover has hopefully been set to the new value the only stuff
    // related to it will be behind extension_version checks past this point
    ...
    if (demover < 200) // Autodetect old demos
    {
        ...
        else
        {
            ...
            // e6y
            // Ability to force -nomonsters and -respawn for playback of 1.2
            // demos. Demos recorded with Doom.exe 1.2 did not contain any
            // information about whether these parameters had been used. In
            // order to play them back, you should add them to the command-line
            // for playback. There is no more desynch on mesh.lmp @ mesh.wad
            // prboom -iwad doom.wad -file mesh.wad -playdemo mesh.lmp
            // -nomonsters http://www.doomworld.com/idgames/index.php?id=13976
    ...
    else // new versions of demos
    {
        ...
        demo_pointer =
            G_ReadOptions(demo_pointer); // killough 3/1/98: Read game options

        if (demover == 200)
        { // killough 6/3/98: partially fix v2.00 demos
...
void G_CheckDemoContinue(void)
{
    if (democontinue)
    {
        ...
        {
            demo_continue_p =
                G_ReadDemoHeaderEx(demobuffer, demolength, RDH_SAFE);
