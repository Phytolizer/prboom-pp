        ...
        if (floor->direction == 1) // going up
        {
            ...
            case genFloorChg0:
                ...
                floor->sector->oldspecial =
                    static_cast<short>(floor->oldspecial);
                // fall thru
            case genFloorChg:
                floor->sector->floorpic = floor->texture;
                break;
...
int EV_BuildStairs(line_t *line, stair_e type)
{
    ...
    manual_stair: // e6y
        ...
        { // jff 2/22/98
            ...
            case build8:
                ...
                if (!demo_compatibility)
                {
                    ...
                    // other value There is no more desync on
                    // icarus.wad/ic29uv.lmp
                    // http://competn.doom2.net/pub/sda/i-o/icuvlmps.zip
                    // http://www.doomworld.com/idgames/index.php?id=5191
                }
                else
                {
                    if (!prboom_comp[PC_UNINITIALIZE_CRUSH_FIELD_FOR_STAIRS]
                             .state)
                    {
            ...
            case heretic_build8:
                speed = FLOORSPEED;
                stairsize = 8 * FRACUNIT;
                crush = STAIRS_UNINITIALIZED_CRUSH_FIELD_VALUE; // heretic_note:
                                                                // I guess
            ...
            //   3. Unless already moving, or different texture, then stop
            //   building
            do
            {
                int i;
                ...
                for (i = 0; i < sec->linecount; i++)
                {
                     ...
                     * comp_stairs, except if we're emulating MBF which
                    ...
                    floor->floordestheight = height;
                    floor->type =
                        buildStair; // jff 3/31/98 do not leave uninited
                    floor->crush =
                        crush; // jff 2/27/98 fix uninitialized crush field
        ...
        if (comp[comp_stairs])
        {
            /* cph 2001/09/22 - emulate buggy MBF comp_stairs for demos, with
            ...
            {
                /* cph 2001/09/22 - now the correct comp_stairs - Doom used a
                 * linear search from the last secnum, so we set that as a
...
int EV_DoDonut(line_t *line)
{
    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        // find a two sided line around the pool whose other side isn't the
        // pillar
        for (i = 0; i < s2->linecount; i++)
        {
            ...
            if (comp[comp_model])
            {
                ...
                // equivalent to:   (!s2->lines[i]->flags) & ML_TWOSIDED , i.e.
                // 0 should be:       !(s2->lines[i]->flags & ML_TWOSIDED)
...
int EV_DoElevator(line_t *line, elevator_e elevtype)
{
    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        rtn = 1;
        elevator = static_cast<elevator_t *>(std::malloc(sizeof(*elevator)));
