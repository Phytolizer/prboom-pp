    ...
    while (ticks--)
    {
        for (i = 0; i < (SCREENWIDTH); i++)
        {
            ...
            if (y_lookup[i] < SCREENHEIGHT)
            {
                ...
                /* cph 2001/07/29 -
                 *  The original melt rate was 8 pixels/sec, i.e. 25 frames to
                 * melt the whole screen, so make the melt rate depend on
                 * SCREENHEIGHT so it takes no longer in high res
                 */
                ...
                if (V_GetMode() != VID_MODEGL)
                {
                    ...
                    d = wipe_scr.data +
                        (y_lookup[i] * wipe_scr.byte_pitch + (i * depth));
                    for (j = dy; j; j--)
                    {
                        for (k = 0; k < depth; k++)
                            d[k] = s[k];
