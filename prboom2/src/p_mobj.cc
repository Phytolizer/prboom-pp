...
static void P_XYMovement(mobj_t *mo)
{
    ...
    do
    {
        ...
        if (!P_TryMove(mo, ptryx, ptryy, true))
        {
            ...
            else if (player || mo->flags2 & MF2_SLIDE)
            ...
            { // whatever else it is, it is now standing still in (x,y)
                mo->momx = mo->momy = 0;
            }
        }
    } while (xmove || ymove);
         ...
         (player->mo != mo && compatibility_level >= lxdoom_1_compatibility)))
    {
        ...
        /* killough 10/98: kill any bobbing momentum too (except in voodoo
         * dolls) cph - DEMOSYNC - needs compatibility check?
         */
    ...
    else
    {
                 ...
                 !prboom_comp[PC_PRBOOM_FRICTION].state)
        {
            ...
            { // No. Use original friction. This allows you to not bob so much
                // if you're on ice, but keeps enough momentum around to break
                // free when you're mildly stuck in a wall.
        ...
        else
        {
             ...
             * This prevents problems with bobbing on ice, where it was not
             * being reduced fast enough, leading to all sorts of kludges being
             * developed.
             */
    ...
    if (mo->flags & MF_BOUNCES && mo->momz)
    {
        ...
        else if (mo->z >= mo->ceilingz - mo->height)
        {
            ...
            if (mo->momz > 0)
            {
                ...
                {
                    mo->momz =
                        -mo->momz; /* always bounce off non-sky ceiling */
                }
                else if (mo->flags & MF_MISSILE)
                {
...
floater:
    ...
    if (mo->z <= mo->floorz)
    {
         ...
         *  if we're in a compatibility level above Doom 2 v1.9 (in which case
         * we mimic the bug and do it further down instead)
         */
        ...
        if (mo->momz < 0)
        {
            ...
            else
            {
                ...
                if (mo->player && /* killough 5/12/98: exclude voodoo dolls */
                    // e6y
                    // Restoring original visual behaviour for
                    // demo_compatibility. Viewheight of consoleplayer should be
                    // decreased for a moment after voodoo doll hits the ground.
                    // This additional condition makes sense only for plutonia
                    // complevel when voodoo doll falls down after teleporting,
    ...
    if (mo->z + mo->height > mo->ceilingz)
    {
        ...
        /* cph 2001/04/15 -
         * We might have hit a ceiling but had downward momentum (e.g. ceiling
         * is lowering on us), so for old demos we must still do the buggy
    ...
    else if (mobj->z != mobj->floorz || mobj->momz)
    {
        if (mobj->flags2 & MF2_PASSMOBJ)
        {
            ...
            else
            {
                ...
                if (mobj->player &&
                    (onmo->player || onmo->type == HERETIC_MT_POD))
                {
                    mobj->momx = onmo->momx;
                    mobj->momy = onmo->momy;
...
mobj_t *P_SpawnMobj(fixed_t x, fixed_t y, fixed_t z, mobjtype_t type)
{
    ...
    else if (heretic && z == FLOATRANDZ)
    {
        ...
        {
            space -= 40 * FRACUNIT;
            mobj->z = ((space * P_Random(pr_heretic)) >> 8) + mobj->floorz +
                      40 * FRACUNIT;
        }
        else
        {
...
static PUREFUNC int P_FindDoomedNum(unsigned type)
{
    ...
    {
        hash =
            static_cast<hash_t *>(std::malloc(sizeof *hash * num_mobj_types));
